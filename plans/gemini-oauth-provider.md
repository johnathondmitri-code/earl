...
## Token Lifecycle
- Store at `~/.earl/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `earl_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `earl_cli/models.py` — Add Gemini model catalog
4. `earl_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `earl_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `earl_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- OR: accept user-provided client_id via EARL_GEMINI_CLIENT_ID env var
...
- earl Copilot OAuth: `earl_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
