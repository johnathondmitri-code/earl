...
Registry and ABC for every inference provider Earl knows about.
...
`$EARL_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `earl_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `earl_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `earl_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `earl_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `earl_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$EARL_HOME/plugins/model-providers/` for a private plugin).
