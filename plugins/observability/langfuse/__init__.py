"""langfuse — Earl plugin for Langfuse observability.

Traces Earl conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Earl plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``earl plugins enable
observability/langfuse``, or by checking the box in the interactive
``earl plugins`` UI). At runtime the plugin also requires the
``langfuse`` SDK and credentials; if either is missing the hooks are inert.

Required env vars (set in ~/.earl/.env):
  EARL_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  EARL_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  EARL_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  EARL_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  EARL_LANGFUSE_RELEASE     - release/version tag
  EARL_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  EARL_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  EARL_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
_LANGFUSE_KEY_PREFIXES: Dict[str, str] = {
    "EARL_LANGFUSE_PUBLIC_KEY": "pk-lf-",
    "EARL_LANGFUSE_SECRET_KEY": "sk-lf-",
}
...
def _debug_enabled() -> bool:
    return _env_bool("EARL_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Earl plugin system —
    ...
    public_key = _env("EARL_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("EARL_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("EARL_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("EARL_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset EARL_LANGFUSE_PUBLIC_KEY / EARL_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("EARL_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("EARL_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("EARL_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("EARL_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid EARL_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("EARL_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Earl branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Earl also has a turn-scoped
    ...
    # messages is a list only for legacy Earl branches that fired
    # pre_llm_call with API messages directly. Current Earl fires
...
def register(ctx) -> None:
    ...
    # Earl versions.  pre_api_request / post_api_request fire per API
