...
```bash
earl memory setup    # select "hindsight"
```
...
```bash
earl config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.earl/.env
```
...
Earl spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.earl/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p earl ui start
```
...
Config file: `~/.earl/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `earl` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `earl-{profile}` isolates memory per active Earl profile. Empty placeholders collapse cleanly (e.g. `earl-{user}` with no user becomes `earl`). |
...
| `retain_context` | `conversation between Earl Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.earl/.env` as `HINDSIGHT_LLM_API_KEY`.
