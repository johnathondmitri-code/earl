...
def _load_supermemory_config(earl_home: str) -> dict:
    config = _default_config()
    config_path = Path(earl_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, earl_home: str) -> None:
    config_path = Path(earl_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `earl memory setup`.
        # All other options are documented for $EARL_HOME/supermemory.json
    ...
    def save_config(self, values, earl_home):
        ...
        _save_supermemory_config(sanitized, earl_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from earl_constants import get_earl_home
        self._earl_home = kwargs.get("earl_home") or str(get_earl_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._earl_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "earl", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "earl_memory", "target": target, "type": "explicit_memory"},
