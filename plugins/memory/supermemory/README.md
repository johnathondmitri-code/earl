...
```bash
earl memory setup    # select "supermemory"
```

Or manually:

```bash
earl config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.earl/.env
```

## Config

Config file: `$EARL_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `earl` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `earl-{identity}` → `earl-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Earl profile:

```json
{
  "container_tag": "earl-{identity}"
}
```

For a profile named `coder`, this resolves to `earl-coder`. The default profile resolves to `earl-default`. Without `{identity}`, all profiles share the same container.
