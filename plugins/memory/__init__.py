...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with earl)
2. User-installed providers: ``$EARL_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$EARL_HOME/plugins/`` or None if unavailable."""
    try:
        from earl_constants import get_earl_home
        d = get_earl_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$EARL_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_earl_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_earl_user_memory.{active_provider}.cli"
    try:
