...
```bash
earl memory setup    # select "byterover"
```

Or manually:
```bash
earl config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.earl/.env
```
...
Working directory: `$EARL_HOME/byterover/` (profile-scoped).
