...
```bash
earl memory setup    # select "holographic"
```

Or manually:
```bash
earl config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.earl-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$EARL_HOME/memory_store.db` | SQLite database path |
