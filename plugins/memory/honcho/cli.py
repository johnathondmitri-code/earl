...
Handles: earl honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "earl" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Earl profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'earl honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'earl honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `earl update` -- no output, no exceptions.
    """
    try:
        from earl_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Earl profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $EARL_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_earl_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Earl persistent cross-session memory.")
    ...
    earl_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'earl honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = earl_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        earl_host["peerName"] = new_peer

    current_ai = earl_host.get("aiPeer") or cfg.get("aiPeer", "earl")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        earl_host["aiPeer"] = new_ai

    current_workspace = earl_host.get("workspace") or cfg.get("workspace", "earl")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        earl_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = earl_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        earl_host["observationMode"] = new_obs
    else:
        earl_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(earl_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        earl_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        earl_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = earl_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        earl_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = earl_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        earl_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                earl_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(earl_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            earl_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        earl_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        earl_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        earl_host["dialecticReasoningLevel"] = new_reasoning
    else:
        earl_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = earl_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        earl_host["sessionStrategy"] = new_strat

    earl_host["enabled"] = True
    earl_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        earl_config = load_config()
        earl_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(earl_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: earl config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    earl honcho status     -- show full config")
    print("    earl honcho mode       -- change recall/observation mode")
    print("    earl honcho tokens     -- tune context and dialectic budgets")
    print("    earl honcho peer       -- update peer names")
    print("    earl honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Earl profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: earl honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'earl honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: earl honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        earl = hosts.get(_host_key(), {})
        user = earl.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = earl.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = earl.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = earl.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Earl' identity in Honcho. Seed with 'earl honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: earl honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: earl honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = earl.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = earl.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = earl.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Earl asks Honcho's AI peer a question")
        ...
        print("\n  Set with: earl honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'earl honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'earl honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    earl honcho identity --show        — show both peer representations")
        print("    earl honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Earl + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Earl\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Earl persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  earl honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'earl honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'earl honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  earl honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'earl'")
        print("  with Honcho configured and no prior session history.")
        print("  (Earl calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    earl honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'earl honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Earl, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'earl honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    earl honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  earl honcho identity <path/to/SOUL.md>")
    ...
    print("    Earl:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Earl:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Earl:   Honcho observes every message and updates representations")
    ...
    print("    Earl:   per-session by default — each run gets its own session")
    print("              Map a custom name:  earl honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. earl honcho setup              — configure API key (required)")
        print("  2. earl honcho migrate            — re-run this walkthrough")
    else:
        print("  1. earl honcho status             — verify Honcho connection")
        print("  2. earl                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. earl honcho identity --show    — verify AI peer representation")
        print("  4. earl honcho tokens             — tune context and dialectic budgets")
        print("  5. earl honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'earl memory setup'...\n")
        from earl_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``earl honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``earl honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to earl memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Earl Honcho",
    )
