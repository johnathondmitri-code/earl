...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/earl>
...
```bash
earl honcho setup    # full interactive wizard (cloud or local)
earl memory setup    # generic picker, also works
```

Or manually:
```bash
earl config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.earl/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$EARL_HOME/honcho.json` | Profile-local (isolated Earl instances) |
| 2 | `~/.earl/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Earl profile: `earl` (default) or `earl.<profile>`.
...
| 4 | `per-session` strategy | Earl session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`earl`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`earl`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-earl`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening earl in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Earl session ID (timestamp + hex). Every `earl` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Earl profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`earl`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Earl profile: `earl` (default) or `earl.<profile>` (e.g. `earl -p coder` → host key `earl.coder`).
...
| `EARL_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `earl honcho setup` | Full interactive setup wizard |
| `earl honcho status` | Show resolved config for active profile |
| `earl honcho enable` / `disable` | Toggle Honcho for active profile |
| `earl honcho mode <mode>` | Change recall or observation mode |
| `earl honcho peer --user <name>` | Update user peer name |
| `earl honcho peer --ai <name>` | Update AI peer name |
| `earl honcho tokens --context <N>` | Set context token budget |
| `earl honcho tokens --dialectic <N>` | Set dialectic max chars |
| `earl honcho map <name>` | Map current directory to a session name |
| `earl honcho sync` | Create host blocks for all Earl profiles |
