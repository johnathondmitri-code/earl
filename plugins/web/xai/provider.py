...
``{title, url, description, position}`` rows every other Earl web
provider produces.
...
prefers Earl-managed xAI Grok OAuth (via ``earl auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.earl/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``earl tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `earl auth` to sign in with "
