...
three virtual model IDs so the ``earl tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `earl tools` → Image "
                    "Generation → OpenAI to configure, or `earl setup` "
                    "to add the key."
                ),
