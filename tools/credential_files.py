...
def _resolve_earl_home() -> Path:
    from earl_constants import get_earl_home
    return get_earl_home()


def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.earl",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``EARL_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside EARL_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    earl_home = _resolve_earl_home()

    # Reject absolute paths — they bypass the EARL_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to EARL_HOME)",
            relative_path,
        )
        return False

    host_path = earl_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape EARL_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, earl_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.earl/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="earl-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching earl_constants.get_earl_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_earl_dir()`` for backward compatibility with old directory layouts.
    """
    from earl_constants import get_earl_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_earl_dir(new_subpath, old_name)
        if host_dir.is_dir():
