...
`mcp_tool.py`, and `earl_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# EarlMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_earl_provider_class() -> Optional[type]:
    ...
    class EarlMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._earl_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`EarlTokenStorage` persisting an absolute
            ...
            from tools.mcp_oauth import EarlTokenStorage
            if (
                isinstance(storage, EarlTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._earl_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import EarlTokenStorage
                        if isinstance(storage, EarlTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._earl_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import EarlTokenStorage
            if not isinstance(storage, EarlTokenStorage):
                return
    ...
    return EarlMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_EARL_PROVIDER_CLS: Optional[type] = _make_earl_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`EarlMCPOAuthProvider` directly using the helpers
        ...
        storage = EarlTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``earl mcp remove <name>`` and (indirectly) by
        ``earl mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
