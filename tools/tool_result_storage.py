...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/earl-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/earl-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/earl-results"
HEREDOC_MARKER = "EARL_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/earl-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"EARL_PERSIST_{uuid.uuid4().hex[:8]}"
