...
Tracks per-skill usage metadata in a sidecar JSON file (~/.earl/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.earl/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.earl/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.earl/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``earl curator
    list-archived`` to help users pass a name to ``earl curator restore``.
    """
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an agent-created skill directory to ~/.earl/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.earl/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.earl/skills/<skill>/SKILL.md) and category-nested
    (~/.earl/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
