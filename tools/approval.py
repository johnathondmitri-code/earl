...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("EARL_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("EARL_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set EARL_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind EARL_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds EARL_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("EARL_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $EARL_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_EARL_ENV_PATH = (
    r'(?:~\/\.earl/|'
    r'(?:\$home|\$\{home\})/\.earl/|'
    r'(?:\$earl_home|\$\{earl_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bearl\s+gateway\s+(stop|restart)\b', "stop/restart earl gateway (kills running agents)"),
    (r'\bearl\s+update\b', "earl update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart earl-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart earl-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(earl|gateway|cli\.py)\b', "kill earl/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill earl` but not
    # `kill -9 $(pgrep -f earl)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("EARL_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("EARL_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("EARL_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("EARL_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("EARL_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("EARL_EXEC_ASK")
