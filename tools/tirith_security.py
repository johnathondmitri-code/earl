...
it is automatically downloaded from GitHub releases to $EARL_HOME/bin/tirith.
...
def _get_earl_home() -> str:
    """Return the Earl home directory, respecting EARL_HOME env var."""
    return str(get_earl_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_earl_home(), ".tirith-install-failed")
...
def _earl_bin_dir() -> str:
    """Return $EARL_HOME/bin, creating it if needed."""
    d = os.path.join(_get_earl_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $EARL_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_earl_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $EARL_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $EARL_HOME/bin/tirith
    ...
    earl_bin = os.path.join(_earl_bin_dir(), "tirith")
    if os.path.isfile(earl_bin) and os.access(earl_bin, os.X_OK):
        _resolved_path = earl_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return earl_bin
