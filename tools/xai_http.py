...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``earl tools`` repaint, tool-registration scans,
    ...
    2. ``~/.earl/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_earl_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.earl/.env`` first, then ``os.environ``.

    Wraps :func:`earl_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from earl_cli.config import get_env_value as _earl_get_env_value

        value = _earl_get_env_value(name)
...
def earl_xai_user_agent() -> str:
    """Return a stable Earl-specific User-Agent for xAI HTTP calls."""
    try:
        from earl_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Earl-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Earl-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``earl_cli.config.get_env_value`` so keys
    stored in ``~/.earl/.env`` (the standard Earl location) are honored —
