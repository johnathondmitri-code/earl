...
One ``CDPSupervisor`` runs per Earl ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "earl-dialog-bridge.invalid"
...
(() => {
  if (window.__earlDialogBridgeInstalled) return;
  window.__earlDialogBridgeInstalled = true;
  const ENDPOINT = "http:/.earl-dialog-bridge.invalid/";
