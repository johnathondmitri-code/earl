# OpenAI-Compatible API Server for Earl Agent
...
SSE streaming. By exposing this endpoint, earl becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.earl/.env`
2. Run `earl gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with earl through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns earl as a model) |
...
Authorization: Bearer earl-api-key-here
...
But earl sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "earl"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured earl model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of earl's core prompt
...
Once implemented, earl works as a drop-in backend for:
