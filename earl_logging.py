"""Centralized logging setup for Earl Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.earl/logs/`` (profile-aware via ``get_earl_home()``).
...
from earl_constants import get_config_path, get_earl_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_earl_session_injector", False):
    ...
    _session_record_factory._earl_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``earl logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "earl_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Earl logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    earl_home
        Override for the Earl home directory.  Falls back to
        ``get_earl_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_earl_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._earl_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the earl group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from earl_cli.config import is_managed
