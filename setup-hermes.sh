...
# 5. Symlinks the 'earl' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Earl Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/earl-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink earl into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up earl command..."

EARL_BIN="$SCRIPT_DIR/venv/bin/earl"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$EARL_BIN" "$COMMAND_LINK_DIR/earl"
echo -e "${GREEN}✓${NC} Symlinked earl → $COMMAND_LINK_DISPLAY_DIR/earl"
...
# ============================================================================
# Seed bundled skills into ~/.earl/skills/
# ============================================================================

EARL_SKILLS_DIR="${EARL_HOME:-$HOME/.earl}/skills"
mkdir -p "$EARL_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.earl/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$EARL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  earl status        # Check configuration"
if is_termux; then
    echo "  earl gateway       # Run gateway in foreground"
else
    echo "  earl gateway install # Install gateway service (messaging + cron)"
fi
echo "  earl cron list     # View scheduled jobs"
echo "  earl doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m earl_cli.main setup
fi
