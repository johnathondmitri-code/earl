# Earl Agent Security Policy

This document describes Earl Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/earl/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Earl Agent does not operate a bug
bounty program.**
...
- Environment details (`earl version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Earl Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Earl Agent's documentation
...
Earl Agent supports two OS-level isolation postures. They address
...
Earl Agent supports this in two ways:

- **Earl Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Earl Agent's in-process heuristics
...
Earl Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Earl Agent
itself. Bugs in Earl Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Earl Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Earl Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Earl Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  earl-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Earl Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Earl Agent. Bugs in Earl
...
- Earl Agent includes supply-chain guards for MCP server
