...
appropriate.  All installs go to a Earl-owned bin staging dir,
``<EARL_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``earl lsp
  status``.
...
# ``<EARL_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def earl_lsp_bin_dir() -> Path:
    """Return the Earl-owned bin staging dir for LSP servers."""
    home = os.environ.get("EARL_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".earl")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = earl_lsp_bin_dir().parent  # <EARL_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a earl-owned target dir.
    ...
    console script.
    """
    pip_target = earl_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = earl_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``earl lsp status`` CLI to give users a quick
