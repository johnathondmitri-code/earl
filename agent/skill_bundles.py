...
Storage
-------
Bundles live in ``~/.earl/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``EARL_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``earl bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under EARL_HOME.

    Honors ``EARL_BUNDLES_DIR`` for tests; falls back to
    ``<EARL_HOME>/skill-bundles``.
    """
    override = os.environ.get("EARL_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_earl_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `earl bundles` CLI subcommand.
# ---------------------------------------------------------------------------
