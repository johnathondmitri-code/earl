...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - earl_home (str): The active EARL_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.earl``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "earl").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'earl memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], earl_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'earl memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``earl_home`` is the active EARL_HOME directory path.
