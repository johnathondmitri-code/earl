...
from earl_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.earl/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from earl_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.earl/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from earl_logging import setup_logging, setup_verbose_logging
    setup_logging(earl_home=_ra()._earl_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that earl_logging.setup_logging() does not
        ...
        # handler level inside earl_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from earl_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `earl model` to "
                        "select a provider, or run `earl setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff EARL_KANBAN_TASK is set).
    ...
    os.environ["EARL_SESSION_ID"] = agent.session_id
    try:
    ...
    # Session logs go into ~/.earl/sessions/ alongside gateway sessions
    earl_home = get_earl_home()
    agent.logs_dir = earl_home / "sessions"
    ...
    try:
        from earl_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "earl"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from earl_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from earl_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Earl Agent.  Choose a model with at least "
