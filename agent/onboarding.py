...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Earl starts and finds ``~/.openclaw/``.

    Points users at ``earl-internal migrate`` (non-destructive port of config,
    memory, and skills) first. ``earl-internal cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Earl, run "
        "`earl-internal migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `earl-internal cleanup` (renames it to ~/.openclaw.pre-migration — "
