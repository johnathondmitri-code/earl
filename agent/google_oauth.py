...
Storage (``~/.earl/auth/google_oauth.json``, chmod 0o600):
...
#
# Resolution order:
#   1. EARL_GEMINI_CLIENT_ID / EARL_GEMINI_CLIENT_SECRET env vars (power users)
...
ENV_CLIENT_ID = "EARL_GEMINI_CLIENT_ID"
ENV_CLIENT_SECRET = "EARL_GEMINI_CLIENT_SECRET"
...
_HEADLESS_ENV_VARS = ("SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "EARL_HEADLESS")
...
def _credentials_path() -> Path:
    return get_earl_home() / "auth" / "google_oauth.json"
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "Earl looks for a locally installed gemini-cli to source the OAuth client. "
            "Either:\n"
            "  1. Install it: npm install -g @google/gemini-cli  (or brew install gemini-cli)\n"
            "  2. Set EARL_GEMINI_CLIENT_ID and EARL_GEMINI_CLIENT_SECRET in ~/.earl/.env\n"
            "\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `earl login --provider google-gemini-cli` first.",
...
<html><head><meta charset="utf-8"><title>Earl — signed in</title>
<style>
...
<html><head><meta charset="utf-8"><title>Earl — sign-in failed</title>
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; margin: 10vh auto; max-width: 32rem; text-align: center; color: #222; }}
h1 {{ color: #b42318; }} p {{ color: #555; }}
</style></head>
<body><h1>Sign-in failed</h1><p>{message}</p>
<p>Return to your terminal — Earl will walk you through a manual paste fallback.</p></body></html>
"""
...
def start_oauth_flow(
    *,
    ...
    auth_url = AUTH_ENDPOINT + "?" + urllib.parse.urlencode(params) + "#earl"
