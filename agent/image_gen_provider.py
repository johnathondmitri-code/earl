...
as ``kind: backend``) or ``~/.earl/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$EARL_HOME/cache/images/``, creating parents as needed."""
    from earl_constants import get_earl_home

    path = get_earl_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$EARL_HOME/cache/images/``.
