...
Every Earl request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Earl Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=earl",
        "client=earl-client-v<__version__>",
    ]

The version is sourced live from ``earl_cli.__version__`` so it auto-aligns
...
``earl_cli.__version__`` is the canonical source of truth.
"""
...
def _earl_version() -> str:
    """Return the current Earl release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``earl_cli`` cannot be imported (should
...
def earl_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=earl-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=earl-client-v{_earl_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=earl", earl_client_tag()]
