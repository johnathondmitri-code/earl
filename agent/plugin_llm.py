...
Plugins built on Earl Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Earl
...
:class:`~earl_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Earl' main config
...
handles every provider, fallback chain, and per-task override Earl
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`earl_cli.plugins.PluginContext`
