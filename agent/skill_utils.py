...
from earl_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``EARL_PLATFORM`` or
            ``EARL_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``earl`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.earl/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``earl`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against EARL_HOME, not cwd
        if not p.is_absolute():
            p = (earl_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.earl/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    earl = metadata.get("earl") or {}
    if not isinstance(earl, dict):
        earl = {}
    return {
        "fallback_for_toolsets": earl.get("fallback_for_toolsets", []),
        "requires_toolsets": earl.get("requires_toolsets", []),
        "fallback_for_tools": earl.get("fallback_for_tools", []),
        "requires_tools": earl.get("requires_tools", []),
    }
