...
from earl_constants import get_earl_home, get_skills_dir, is_wsl
...
_EARL_MD_NAMES = (".earl.md", "EARL.md")


def _find_earl_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.earl.md`` or ``EARL.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Earl Agent, an intelligent AI assistant created by Nous Research. "
...
EARL_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Earl Agent "
    "itself, load the `earl` skill with skill_view(name='earl') "
    "before answering. Docs: https:/.earl-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.earl/kanban.db`. Your task id is in "
    "`$EARL_KANBAN_TASK`; your workspace is `$EARL_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $EARL_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `earl kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Earl WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Earl itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Earl runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Earl itself is running. The host OS, home, and cwd "
                f"of the Earl process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Earl "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_earl_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.earl/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("EARL_PLATFORM")
        or get_session_env("EARL_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Earl Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `earl` skill "
            "first. It has the actual commands (e.g. `earl config set …`, `earl tools`, "
            "`earl setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: earl setup, earl setup tools, earl setup terminal, earl status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from EARL_HOME and return its content, or None.
    ...
    """
    try:
        from earl_cli.config import ensure_earl_home
        ensure_earl_home()
    except Exception as e:
        logger.debug("Could not ensure EARL_HOME before loading SOUL.md: %s", e)

    soul_path = get_earl_home() / "SOUL.md"
...
def _load_earl_md(cwd_path: Path) -> str:
    """.earl.md / EARL.md — walk to git root."""
    earl_md_path = _find_earl_md(cwd_path)
    if not earl_md_path:
        return ""
    try:
        content = earl_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = earl_md_path.name
        try:
            rel = str(earl_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".earl.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", earl_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .earl.md / EARL.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from EARL_HOME is independent and always included when present.
    ...
    # SOUL.md from EARL_HOME only — skip when already loaded as identity
    if not skip_soul:
