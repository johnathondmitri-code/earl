...
def _state_file() -> Path:
    return get_earl_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.earl/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``earl update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``earl curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `earl curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Earl' background skill CURATOR. This is an "
    ...
    "into ~/.earl/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: earl-config-*, earl-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.earl/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.earl/logs/curator/``)
    ...
    authored skill data in ``~/.earl/skills/``.

    ``ensure_earl_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_earl_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every earl install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: earl curator status
        keep an umbrella stable: earl curator pin document-tools
    ...
    lines.append("full report: earl curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: earl curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.earl/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.earl/skills/.archive/` for "
            "safety and can be restored via `earl curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.earl/skills/.archive/`. "
            "Restore any via `earl curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `earl curator restore <name>`")
    lines.append("- All archives live under `~/.earl/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `earl curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `earl curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``earl model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `earl model` → auxiliary
