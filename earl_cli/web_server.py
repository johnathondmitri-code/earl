"""
Earl Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m earl_cli.main web          # Start on http://127.0.0.1:9119
    python -m earl_cli.main web --port 8080
"""
...
try:
    ...
    # running `earl dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["EARL_WEB_DIST"]) if "EARL_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Earl Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Earl-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``earl dashboard --tui``
# or EARL_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.earl/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_earl_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "earl-update": "earl-update.log",
}
...
def _spawn_earl_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``earl <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``earl_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "earl_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "EARL_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``earl gateway restart`` in the background."""
    try:
        proc = _spawn_earl_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/earl/update")
async def update_earl():
    """Kick off ``earl update`` in the background."""
    try:
        proc = _spawn_earl_action(["update"], "earl-update")
    except Exception as exc:
        _log.exception("Failed to spawn earl update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Earl supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in earl_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.earl/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``earl auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Earl resolves Anthropic creds in this order at runtime:
    1. ``~/.earl/.anthropic_oauth.json`` — Earl-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_earl_oauth_credentials,
            read_claude_code_credentials,
            _EARL_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_earl_oauth_credentials = None  # type: ignore
        _EARL_OAUTH_FILE = None  # type: ignore

    earl_creds = None
    if read_earl_oauth_credentials:
        try:
            earl_creds = read_earl_oauth_credentials()
        except Exception:
            earl_creds = None
    if earl_creds and earl_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "earl_pkce",
            "source_label": f"Earl PKCE ({_EARL_OAUTH_FILE})",
            "token_preview": _truncate_token(earl_creds.get("accessToken")),
            "expires_at": earl_creds.get("expiresAt"),
            "has_refresh_token": bool(earl_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Earl even
    when they also have a separate Earl-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "earl auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "earl auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "earl auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "earl auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "earl auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from earl_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("earl_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Earl-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _EARL_OAUTH_FILE
            if _EARL_OAUTH_FILE.exists():
                _EARL_OAUTH_FILE.unlink()
        except Exception:
            pass
...
#          → persists to ~/.earl/.anthropic_oauth.json AND credential pool
...
# Guarded so earl web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Earl file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``earl auth add anthropic``.
    """
    from agent.anthropic_adapter import _EARL_OAUTH_FILE
    payload = {
    ...
    _EARL_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _EARL_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("EARL_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from earl_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from earl_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_earl_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from earl_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from earl_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, EARL_HOME)."""
    from earl_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process EARL_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_earl_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "earl setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from earl_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from earl_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``earl --tui`` binary the CLI uses, behind
...
try:
    from earl_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``earl --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``EARL_TUI_RESUME`` env var —
    matching what ``earl_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``EARL_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from earl_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("EARL_TUI_DISABLE_MOUSE", "1")
    env.setdefault("EARL_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["EARL_TUI_RESUME"] = resume

    if sidecar_url:
        env["EARL_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Earl inside WSL2 to use the dashboard's /chat "
...
# EARL_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/earl"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/earl/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /earl`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__EARL_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/earl``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__EARL_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__EARL_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__EARL_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/earl`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Earl backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Earl Teal",         "description": "Classic dark teal — the canonical Earl look"},
    {"name": "default-large", "label": "Earl Teal (Large)", "description": "Earl Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.earl/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.earl/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_earl_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.earl/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as earl_cli.plugins):
    1. User plugins:    ~/.earl/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.earl/plugins/  (only if EARL_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from earl_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_earl_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("EARL_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".earl" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from earl_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_earl_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from earl_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from earl_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from earl_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from earl_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"earl_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Earl Web UI → http://{host}:{port}")
