"""Shell completion script generation for earl CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_earl_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Earl Agent bash completion
# Add to ~/.bashrc:
#   eval "$(earl completion bash)"

_earl_profiles() {{
    local profiles_dir="$HOME/.earl/profiles"
...
_earl_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_earl_profiles)" -- "$cur"))
        return
    fi
...
complete -F _earl_completion earl
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef earl
# Earl Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(earl completion zsh)"

_earl_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.earl/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.earl/profiles 2>/dev/null)}}")
    fi
...
_earl() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_earl_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'earl command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Earl Agent fish completion",
        "# Add to your config:",
        "#   earl completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __earl_profiles",
        "    echo default",
        "    if test -d $HOME/.earl/profiles",
        "        ls $HOME/.earl/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c earl -f -s p -l profile"
        " -d 'Profile name' -xa '(__earl_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__earl_profiles)' -d 'Profile name'"
                )
