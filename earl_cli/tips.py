...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or earl -c.",
    ...
    "Ctrl+Z suspends Earl to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "earl -c resumes your most recent CLI session. earl -c \"project name\" resumes by title.",
    "earl -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "earl -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "earl chat -t web,terminal enables only specific toolsets for a focused session.",
    "earl chat -s github-pr-workflow preloads a skill at launch.",
    "earl chat -q \"query\" runs a single non-interactive query and exits.",
    "earl chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "earl chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "earl --yolo bypasses all dangerous command approval prompts for the entire session.",
    "earl chat --source telegram tags the session for filtering in earl sessions list.",
    "earl -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "earl doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "earl dump outputs a compact setup summary — great for bug reports.",
    "earl config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "earl config edit opens config.yaml in your default editor.",
    "earl config check scans for missing or stale configuration options.",
    "earl sessions browse opens an interactive session picker with search.",
    "earl sessions stats shows session counts by platform and database size.",
    "earl sessions prune --older-than 30 cleans up old sessions.",
    "earl skills search react --source skills-sh searches the skills.sh public directory.",
    "earl skills check scans installed hub skills for upstream updates.",
    "earl skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "earl skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "earl mcp add github --command npx adds MCP servers from the command line.",
    "earl mcp serve runs Earl itself as an MCP server for other agents.",
    "earl auth add lets you add multiple API keys for credential pool rotation.",
    "earl completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "earl logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "earl backup creates a zip backup of your entire Earl home directory.",
    "earl profile create coder creates an isolated profile that becomes its own command.",
    "earl profile create work --clone copies your current config and keys to a new profile.",
    "earl update syncs new bundled skills to ALL profiles automatically.",
    "earl gateway install sets up Earl as a system service (systemd/launchd).",
    "earl memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "earl webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: earl tools disables unused tools, earl skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "earl models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as earl -w).",
    ...
    "EARL_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Earl tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'earl profile create coder' creates the 'coder' command.",
    "earl profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Earl prints a resume command with session ID and stats.",
    "earl sessions export backup.jsonl exports all sessions for backup or analysis.",
    "earl -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "earl skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .earl/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: earl cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Earl runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "earl gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Earl via a local macOS server.",
    ...
    "Context files (.earl.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.earl/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Earl loads project context from .earl.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — earl tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.earl/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "earl chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "earl plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "earl acp runs Earl as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "EARL_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "earl login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "earl status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.earl/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Earl your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.earl/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Earl auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.earl/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at EARL_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "EARL_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in EARL_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.earl/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'earl profile create ls' would be rejected.",
    "earl profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "earl claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "EARL_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via earl tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.earl/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.earl/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Earl auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Earl config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Earl is working.',
    ...
    '/reload reloads ~/.earl/.env into the running session — pick up new API keys without restarting.',
    ...
    "EARL_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.earl/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.earl/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'earl curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "earl curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'earl curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'earl auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'EARL_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "EARL_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'earl dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'earl dashboard --tui embeds the full Earl TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.earl/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.earl/dashboard-plugins/ — no npm build required.',
    ...
    'EARL_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'EARL_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'EARL_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'EARL_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.earl/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.earl/skills/ so drop-in skills appear without restarting the session.',
    ...
    'earl -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'earl chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'earl chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'earl chat --ignore-user-config skips ~/.earl/config.yaml — reproducible bug reports and CI runs.',
    "earl chat --source tool tags programmatic chats so they don't clutter earl sessions list.",
    'earl dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'earl sessions rename <ID> "new title" renames any past session; earl sessions delete <ID> removes one.',
    'earl import restores a session export or profile archive produced by sessions export or profile export.',
    'earl fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'earl pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'earl setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'earl status --deep runs the full health sweep across every component; plain earl status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'EARL_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'EARL_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.earl/plugins/ — trust-gated by design.',
    "EARL_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'EARL_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'EARL_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'EARL_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'EARL_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'EARL_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'EARL_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'EARL_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'EARL_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'EARL_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'EARL_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Earl block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from earl sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.earl/dashboard-plugins/.',
]
