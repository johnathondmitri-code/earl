"""``earl logs`` — view and filter Earl log files.
...
Usage examples::
    ...
    earl logs                    # last 50 lines of agent.log
    earl logs -f                 # follow agent.log in real time
    earl logs errors             # last 50 lines of errors.log
    earl logs gateway -n 100    # last 100 lines of gateway.log
    earl logs --level WARNING    # only WARNING+ lines
    earl logs --session abc123   # filter by session ID substring
    earl logs --component tools  # only tool-related lines
    earl logs --since 1h         # lines from the last hour
    earl logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_earl_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Earl runs — try 'earl chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_earl_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_earl_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_earl_home()}/logs/")
        return

    print(f"Log files in {display_earl_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'earl chat' to generate logs)")
