"""`earl checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.earl/checkpoints/``.  Actions:

    earl checkpoints               # same as `status`
    earl checkpoints status        # total size, project count, breakdown
    earl checkpoints list          # per-project checkpoint counts + workdir
    earl checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    earl checkpoints clear [-f]    # nuke the entire base (asks first)
    earl checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    earl checkpoints
    earl checkpoints prune --retention-days 3 --max-size-mb 200
    earl checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: earl checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``earl checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `earl checkpoints` → status
