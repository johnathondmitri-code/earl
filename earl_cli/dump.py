"""
Dump command for earl CLI.

Outputs a compact, plain-text summary of the user's Earl setup
...
from earl_cli.config import get_earl_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``earl dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from earl_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(earl_home: Path) -> int:
...
def _cron_summary(earl_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = earl_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"earl_home:      {display_earl_home()}")
    ...
    toolsets = config.get("toolsets", ["earl-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(earl_home)}")
    lines.append(f"  skills:             {_count_skills(earl_home)}")
