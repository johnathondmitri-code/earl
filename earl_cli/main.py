#!/usr/bin/env python3
"""
Earl CLI - Main entry point.

Usage:
    earl                     # Interactive chat (default)
    earl chat                # Interactive chat
    earl gateway             # Run gateway in foreground
    earl gateway start       # Start gateway as service
    earl gateway stop        # Stop gateway service
    earl gateway status      # Show gateway status
    earl gateway install     # Install gateway service
    earl gateway uninstall   # Uninstall gateway service
    earl setup               # Interactive setup wizard
    earl logout              # Clear stored authentication
    earl status              # Show status of all components
    earl cron                # Manage cron jobs
    earl cron list           # List cron jobs
    earl cron status         # Check if cron scheduler is running
    earl doctor              # Check configuration and dependencies
    earl honcho setup                    # Configure Honcho AI memory integration
    earl honcho status                   # Show Honcho config and connection status
    earl honcho sessions                 # List directory → session name mappings
    earl honcho map <name>               # Map current directory to a session name
    earl honcho peer                     # Show peer names and dialectic settings
    earl honcho peer --user NAME         # Set user peer name
    earl honcho peer --ai NAME           # Set AI peer name
    earl honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    earl honcho mode                     # Show current memory mode
    earl honcho mode [hybrid|honcho|local]  # Set memory mode
    earl honcho tokens                   # Show token budget settings
    earl honcho tokens --context N       # Set session.context() token cap
    earl honcho tokens --dialectic N     # Set dialectic result char cap
    earl honcho identity                 # Show AI peer identity representation
    earl honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    earl honcho migrate                  # Step-by-step migration guide: OpenClaw native → Earl + Honcho
    earl version             Show version
    earl update              Update to latest version
    earl uninstall           Uninstall Earl Agent
    earl acp                 Run as an ACP server for editor integration
    earl sessions browse     Interactive session picker with search

    earl claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: earl_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``earl_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``earl update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``earl_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, earl crashes on import and the user can't run
# ``earl update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import earl_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to EARL_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (earl tools, earl setup, earl model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'earl {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any earl module import.
#
# Many modules cache EARL_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("EARL_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.earl/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set EARL_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors earl_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If EARL_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.earl/profiles/coder or
    # /opt/data/profiles/coder).  If EARL_HOME points to the earl root
    # instead (e.g. systemd hardcodes EARL_HOME=/root/.earl), we must
    # still read active_profile — the user may have switched profiles via
    # `earl profile use` and the gateway should honour that choice.
    # See issue #22502.
    earl_home_env = os.environ.get("EARL_HOME", "")
    if profile_name is None and earl_home_env:
        if Path(earl_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the earl root
    if profile_name is None:
        try:
            from earl_constants import get_default_earl_root

            active_path = get_default_earl_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set EARL_HOME
    if profile_name is not None:
        try:
            from earl_cli.profiles import resolve_profile_env

            earl_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent earl from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["EARL_HOME"] = earl_home
...
# Load .env from ~/.earl/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from earl_cli.config import get_earl_home
from earl_cli.env_loader import load_earl_dotenv

load_earl_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → EARL_REDACT_SECRETS env
# var BEFORE earl_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "EARL_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_earl_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["EARL_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `earl` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from earl_logging import setup_logging as _setup_logging
...
try:
    from earl_cli.config import load_config as _load_config_early
    from earl_constants import apply_ipv4_preference as _apply_ipv4
...
from earl_cli import __version__, __release_date__
from earl_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from earl_cli.config import get_env_path, get_earl_home, load_config
    from earl_cli.auth import get_auth_status

    # Determine whether Earl itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from earl_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_earl_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from earl_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_earl_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Earl has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Earl to use their tokens.
    if _has_earl_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, earl_bin
        cli_args: the original CLI arguments (everything after 'earl')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    earl_bin = container_info["earl_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo earl {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo earl {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  earl --tui --resume {target}")
    if title:
        print(f'  earl --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @earl/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@earl" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``EARL_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("EARL_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    earl_home = os.environ.get("EARL_HOME") or str(Path.home() / ".earl")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "EARL_HOME": earl_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(earl_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(earl_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if earl_cli_dir is None:
        earl_cli_dir = Path(__file__).parent
    bundled = earl_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (EARL_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("EARL_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from earl_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("EARL_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with EARL_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset EARL_TUI_DIR (e.g. `unset EARL_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("EARL_QUIET"):
    ...
    if tui_dev:
        # Keep the local @earl/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @earl/ink resolves through
        # packages/earl-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "earl-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from earl_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="earl-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["EARL_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["EARL_PYTHON_SRC_ROOT"] = os.environ.get(
        "EARL_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("EARL_PYTHON", sys.executable)
    env.setdefault("EARL_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["EARL_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["EARL_MODEL"] = model
        env["EARL_INFERENCE_MODEL"] = model
    if provider:
        env["EARL_TUI_PROVIDER"] = provider
        env["EARL_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["EARL_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["EARL_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["EARL_TUI_SKILLS"] = value
    if query:
        env["EARL_TUI_QUERY"] = query
    if image:
        env["EARL_TUI_IMAGE"] = image
    if checkpoints:
        env["EARL_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["EARL_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["EARL_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["EARL_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["EARL_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["EARL_ACCEPT_HOOKS"] = "1"
    ...
    # EARL_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `earl --tui` try to
    ...
    # still set EARL_TUI_RESUME themselves.
    env.pop("EARL_TUI_RESUME", None)
    if resume_session_id:
        env["EARL_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `earl update` so
    ...
    if code == 42:
        from earl_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``EARL_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``earl kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``earl kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("EARL_KANBAN_BOARD"):
        return
    try:
        from earl_cli.kanban_db import get_current_board

        os.environ["EARL_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("EARL_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'earl sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Earl isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  earl setup")
        print()

        from earl_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'earl setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from earl_cli.banner import prefetch_update_check
    ...
    # user's ~/.earl/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["EARL_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["EARL_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["EARL_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from earl_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from earl_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from earl_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Earl?")
        print()
    ...
    # has no creds.json.  Every subsequent `earl gateway` then paid a 30s
    ...
    session_dir = get_earl_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: earl gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `earl gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  earl gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Earl Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  earl gateway")
        ...
        print()
        print("  Or install as a service: earl gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'earl whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from earl_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from earl_cli.config import stamp_install_method
    from earl_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Earl post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``earl model``) and the setup wizard
    ...
    from earl_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from earl_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("EARL_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from earl_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'earl model' for "
                "available providers, or run 'earl doctor' to diagnose config "
                "issues."
            )
    ...
    from earl_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.earl/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.earl/.env if the active provider is not 'custom'.
    ...
    """
    from earl_cli.config import get_env_value, save_env_value, load_config
...
#
# Earl uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `earl model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `earl model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from earl_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from earl_cli.config import load_config
    ...
    while True:
        ...
        print("  Earl only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``earl model`` flow, then route aux tasks to them here.
    """
    from earl_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from earl_cli.auth import _prompt_model_selection
    from earl_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from earl_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from earl_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.earl/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `earl setup` from scratch. OpenRouter
    ...
    from earl_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.earl/.env by hand.
    ...
    from earl_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from earl_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from earl_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from earl_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `earl auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from earl_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from earl_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``earl model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``earl model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``earl auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from earl_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from earl_cli.auth import _save_model_choice, deactivate_provider
    from earl_cli.config import get_env_value, load_config, save_config
    ...
    from earl_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `earl model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from earl_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Earl URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from earl_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from earl_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from earl_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Earl will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Earl will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from earl_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from earl_cli.config import save_env_value, load_config, save_config
    from earl_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from earl_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from earl_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Earl turns to `copilot --acp`.")
    print("  Earl currently starts its own ACP subprocess for each request.")
    print("  Earl uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set EARL_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``earl setup`` / ``earl model``.
    ...
    recover from a malformed paste without editing ``~/.earl/.env`` by hand.
    ...
    from earl_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from earl_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `earl setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so earl knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Earl typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Earl, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from earl_cli.auth import AuthError
        from earl_cli.models import fetch_lmstudio_models
        ...
        from earl_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from earl_constants import display_earl_home as _dhh_fn

            print(
                f"    Earl will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from earl_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'earl model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Earl CLI with a provider."""
    from earl_cli.auth import login_command
...
def cmd_auth(args):
    ...
    from earl_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from earl_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from earl_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from earl_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``earl slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: earl slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `earl slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from earl_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from earl_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from earl_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from earl_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from earl_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from earl_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from earl_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Earl home directory to a zip file."""
    if getattr(args, "quick", False):
        from earl_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a Earl backup from a zip file."""
...
def cmd_version(args):
    """Show version."""
    print(f"Earl Agent v{__version__} ({__release_date__})")
    ...
    try:
        from earl_cli.banner import check_for_updates
        from earl_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Earl Agent."""
    _require_tty("uninstall")
    from earl_cli.uninstall import run_uninstall
...
# Critical files that every ``earl`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``earl update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "earl_cli/main.py",
    "earl_cli/config.py",
    "earl_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``earl`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``earl update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``earl_cli/web_dist/`` (per vite.config.ts
    outDir: "../earl_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "earl_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``earl update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``earl web``).
    ...
    # build path usable on a stock `py -m earl_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (earl web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``earl dashboard`` processes other than ourselves.

    ``earl dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``earl update`` replaces files on disk, the running
    ...
    patterns = [
        "earl dashboard",
        "earl_cli.main dashboard",
        "earl_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "earl.*dashboard"`) keeps us consistent
            # with `earl_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `earl update`.
    ...
    print("  Preview now:  earl curator run --dry-run")
    print("  Pause it:     earl curator pause")
    print(
        "  Docs:         https:/.earl-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``earl update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``earl update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: earl curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``earl dashboard`` processes.

    Called at the end of ``earl update`` (default ``reason``) and also
    from ``earl dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    earl dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Earl Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/earl/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="earl-update-")
        zip_path = os.path.join(tmp_dir, f"earl-{branch}.zip")
        ...
        # GitHub ZIPs extract to earl-<branch>/
        extracted = os.path.join(tmp_dir, f"earl-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "earl-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Earl behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes earl completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Earl couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Earl couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Earl behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `earl update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/earl.git",
    "git@github.com:NousResearch/earl.git",
    "https://github.com/NousResearch/earl",
    "git@github.com:NousResearch/earl",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/earl.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_earl_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_earl_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Earl repository.")
        print("  This means you may miss updates from NousResearch/earl.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/earl.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/earl.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``earl update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from earl_constants import get_default_earl_root

    default_home = get_default_earl_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``earl update`` is still progressing even if pip/uv itself is silent.
    """
...
def _earl_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "earl.exe",
        scripts_dir / "earl-gateway.exe",
    ]


def _detect_concurrent_earl_instances(
    ...
    The Earl Desktop Electron app spawns ``earl.EXE`` as a backend child,
    so during ``earl update`` the user-invoked process and the desktop's
    ...
    shims (``earl.exe`` / ``earl-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID is excluded so the running
    ``earl update`` invocation never reports itself.
    ...
    for shim in _earl_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    lines = ["✗ Another earl.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Earl Desktop, exit any open `earl` REPLs, and")
    lines.append("  stop the gateway (`earl gateway stop`) before retrying.")
    lines.append("  Override with `earl update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_earl_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``earl.exe``.
    ...
    when ``earl update`` runs, ``earl.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``earl.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next earl invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Earl Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Earl Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _earl_exe_shims(scripts_dir):
        ...
        print(
            "    Close Earl Desktop, exit other `earl` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `earl update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_earl_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``earl.exe.old.*`` left by prior updates.

    Called early on every earl invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``earl update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `earl update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``earl.exe`` / ``earl-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_earl_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_earl_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.earl/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``earl update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.earl/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``earl update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``earl update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.earl/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``earl update --gateway``) the update is already
    ...
    try:
        ...
        # earl_cli.config.get_earl_home to simulate setup failure.
        from earl_cli.config import get_earl_home as _get_earl_home

        logs_dir = _get_earl_home() / "logs"
...
def _cmd_update_check():
    """Implement ``earl update --check``: fetch and report without installing."""
    from earl_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from earl_cli.config import recommended_update_command
        from earl_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``earl update`` without requiring a reinstall.  The
    ...
    ``earl`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/earl.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v earl'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/earl, code at /usr/local/lib/earl).
    fhs_link = Path("/usr/local/bin/earl")
    ...
    path_comment = (
        "# Earl Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of EARL_HOME before running the update.
    ...
    EARL_HOME directories.  The ``--backup`` flag on ``earl update``
    ...
    # Render path using display_earl_home so the user sees ~/.earl/...
    try:
        from earl_constants import get_earl_home, display_earl_home

        home = get_earl_home()
        try:
            display_path = f"{display_earl_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  earl import {out_path}")
...
def cmd_update(args):
    """Update Earl Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from earl_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Earl Agent")
        return
...
def _cmd_update_pip(args):
    """Update Earl via pip (for PyPI installs)."""
    from earl_cli import __version__
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "earl"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "earl"]
    ...
    print("✓ Update complete! Restart earl to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Earl Agent...")
    print()

    # On Windows, abort early if another earl.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_earl_instances(scripts_dir)
            if concurrent:
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/earl/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # though `git pull` can't touch $EARL_HOME, this is cheap
        ...
        # orphan merge-conflict markers in earl_cli/config.py bricked
        # every user who ran ``earl update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``earl
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``earl update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_earl_home added to earl_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload earl_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_earl_home() added since the last release.
        try:
            import importlib
            import earl_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit EARL_HOME so
        # it is not affected by sync_skills()'s module-level EARL_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's EARL_HOME env var points at the default or a named profile.
        try:
            from earl_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: earl config migrate")
            else:
                print()
                print("Skipped. Run 'earl config migrate' later to configure.")
        else:
        ...
        # check `earl curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``earl update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from earl_cli.tools_config import install_cua_driver
        ...
        # When running as ``earl update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_earl_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all earl-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. earl-gateway.service or earl-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: earl gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: earl -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Earl gateway unit files are still installed.
        # When both earl.service (from a pre-rename install) and the
        # current earl-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `earl update` surfaces the issue until the user migrates.
        try:
            from earl_cli.gateway import (
                has_legacy_earl_units,
                _find_legacy_earl_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_earl_units():
                print()
                print("⚠ Legacy Earl gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_earl_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (earl.service) fight the current")
                print("  earl-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    earl gateway migrate-legacy")
                print()
        ...
        print("  earl model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``earl -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from earl_constants import display_earl_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `earl profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_earl_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → earl -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.earl)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  earl profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     earl -p {name} chat")
                else:
        ...
        from earl_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from earl_constants import get_earl_home as _hh
        ...
        from earl_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `earl profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from earl_cli.profiles import profile_exists
        ...
        from earl_cli.profiles import rename_profile
        ...
        from earl_cli.profiles import export_profile
        ...
        from earl_cli.profiles import import_profile
        ...
        import tempfile
        from earl_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="earl_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  earl -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      earl -p {plan.manifest.name} chat")
        ...
        from earl_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `earl profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"earl -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from earl_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("earl_requires"):
            print(f"Requires:     Earl {data['earl_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from earl_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.earl_requires:
        print(f"  Requires: Earl {mf.earl_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``earl dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``earl dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No earl dashboard processes running.")
        return 0

    print(f"{len(pids)} earl dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No earl dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `earl update`.
    ...
    if "EARL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["EARL_WEB_DIST"])
            if "EARL_WEB_DIST" in os.environ
            else PROJECT_ROOT / "earl_cli" / "web_dist"
        )
    ...
    from earl_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("EARL_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from earl_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Earl log files."""
    from earl_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from earl_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``earl -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``earl_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``earl -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``earl`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for earl CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from earl_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``earl.exe.old.*`` quarantine files left by previous
    # ``earl update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_earl_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from earl_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: earl-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from earl_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https:/.earl-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `earl model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy earl.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Earl gateway unit files "
            "(e.g. earl.service) left over from older installs. Profile "
            "units (earl-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `earl proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Earl Agent with an interactive wizard. "
        "Run a specific section: earl setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'earl setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Earl.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $EARL_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Earl")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from earl_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Earl CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: earl-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Earl with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set EARL_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Earl Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.earl/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Earl profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.earl/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Earl profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from earl_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.earl/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.earl/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Earl Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `earl "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Earl setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Earl Agent. Use 'earl debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    earl debug share              Upload debug report and print URL
    earl debug share --lines 500  Include more log lines
    earl debug share --expire 30  Keep paste for 30 days
    earl debug share --local      Print report locally (no upload)
    earl debug share --no-redact  Disable upload-time secret redaction
    earl debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'earl debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Earl home directory to a zip file",
        description="Create a zip archive of your entire Earl configuration, "
        "skills, sessions, and data (excludes the earl codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/earl-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.earl/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo earl uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from earl_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Earl backup from a zip file",
        description="Extract a previously created Earl backup into your "
        "Earl home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Earl Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from earl_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.earl/skills/.bundled_manifest) "
            "so future 'earl update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from earl_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from earl_cli.skills_hub import skills_command
    ...
    from earl_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/earl-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `earl plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from earl_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``earl --help``, ``earl version``, ``earl logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from earl_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from earl_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_earl_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_earl_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_earl_home()}/memories/\n")
        else:
            from earl_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'earl tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # earl tools disable <name...> [--platform cli]
    ...
    # earl tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from earl_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from earl_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `earl computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `earl tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: earl computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: earl computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Earl as an MCP server",
        description=(
            "Manage MCP server connections and run Earl as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'earl mcp add' to connect to a new server, or\n"
            "'earl mcp serve' to expose Earl conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Earl as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `earl mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from earl_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_earl_home() / "sessions"
            ...
            # Launch earl --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Earl",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Earl",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.earl/ (by default a "
        "single restore-point archive is written to ~/.earl/backups/ "
        "before apply; restorable with 'earl import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from earl_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Earl Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'earl config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--force",
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another earl.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Earl Agent",
        description="Remove Earl Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Earl Agent as an ACP (Agent Client Protocol) server",
        description="Start Earl Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Earl ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Earl provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.earl/node/ "
    ...
    def cmd_acp(args):
        """Launch Earl Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Earl instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `earl update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `earl profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Earl profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Earl Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `earl --tui` via PTY/WebSocket). "
            "Alternatively set EARL_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `earl dashboard` cmdlines and SIGTERM them directly — the same
    # path `earl update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running earl dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running earl dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Earl log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    earl logs                    Show last 50 lines of agent.log
    earl logs -f                 Follow agent.log in real time
    earl logs errors             Show last 50 lines of errors.log
    earl logs gateway -n 100     Show last 100 lines of gateway.log
    earl logs --level WARNING    Only show WARNING and above
    earl logs --session abc123   Filter by session ID
    earl logs --component tools  Only show tool-related lines
    earl logs --since 1h         Lines from the last hour
    earl logs --since 30m -f     Follow, starting from 30 min ago
    earl logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``earl -c Pokemon Agent Dev`` → ``earl -c 'Pokemon Agent Dev'``
    ...
    from earl_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'earl -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (earl hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from earl_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from earl_cli.oneshot import run_oneshot
