...
Usage:
    earl pairing list              # Show all pending + approved users
    earl pairing approve <platform> <code>  # Approve a pairing code
    earl pairing revoke <platform> <user_id> # Revoke user access
    earl pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle earl pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: earl pairing {list|approve|revoke|clear-pending}")
        print("Run 'earl pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.earl/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'earl pairing list' to see pending codes.\n")
