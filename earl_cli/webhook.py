"""earl webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    earl webhook subscribe <name> [options]
    earl webhook list
    earl webhook remove <name>
    earl webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.earl/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _earl_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: earl gateway run
"""
...
def webhook_command(args):
    """Entry point for 'earl webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: earl webhook {subscribe|list|remove|test}")
        print("Run 'earl webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (earl gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: earl webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from earl webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (earl gateway run)")
