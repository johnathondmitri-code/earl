"""
Backup and import commands for earl CLI.

`earl backup` creates a zip archive of the entire ~/.earl/ directory
(excluding the earl repo and transient files).

`earl import` restores from a backup zip, overlaying onto the current
EARL_HOME root.
"""
...
from earl_constants import get_default_earl_root, get_earl_home, display_earl_home
...
_EXCLUDED_DIRS = {
    "earl",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to earl root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Earl home directory."""
    earl_root = get_default_earl_root()

    if not earl_root.is_dir():
        print(f"Error: Earl home directory not found at {earl_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"earl-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"earl-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_earl_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(earl_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(earl_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(earl_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside earl root
            try:
    ...
    print(f"\nRestore with: earl import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Earl backup.
    ...
    # Look for telltale files that a earl home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Earl backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.earl/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a earl dir name
        if prefix in {".earl", "earl"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_earl_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (earl_root / "config.yaml").exists()
        has_env = (earl_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Earl configuration.")
        ...
        earl_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = earl_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(earl_root.resolve())
            except ValueError:
        ...
        profiles_dir = earl_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # earl_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: earl profile list  (after installing earl)")

        # Guidance
        print()
        if not (earl_root / "earl").is_dir():
            print("Note: The earl codebase was not included in the backup.")
            print("  If this is a fresh install, run: earl update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  earl -p {pname} gateway install")

        print("Done. Your Earl configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and earl backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to EARL_HOME).
...
# — `earl update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(earl_home: Optional[Path] = None) -> Path:
    home = earl_home or get_earl_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    earl_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(earl_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(earl_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for earl backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_earl_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, earl_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``earl_root`` to ``out_path``.
...
def _pre_update_backup_dir(earl_home: Optional[Path] = None) -> Path:
    home = earl_home or get_earl_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of EARL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<EARL_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``earl update``) should continue even if the backup fails.
    """
    earl_root = earl_home or get_default_earl_root()
    if not earl_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(earl_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, earl_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `earl-internal migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of EARL_HOME under ``backups/`` before a
    ``earl-internal migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``earl import <archive>``.  Writes to
    ``<EARL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `earl import` and the
