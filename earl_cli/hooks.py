"""earl hooks — inspect and manage shell-script hooks.

Usage::

    earl hooks list
    earl hooks test <event> [--for-tool X] [--payload-file F]
    earl hooks revoke <command>
    earl hooks doctor

Consent records live under ``~/.earl/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.earl/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``earl hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: earl hooks {list|test|revoke|doctor}")
        print("Run 'earl hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.earl/config.yaml.")
        print("See `earl hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `earl hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `earl hooks test` and `earl hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Earl wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `earl hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `earl hooks doctor` would execute
