"""
Single source of truth for provider identity in Earl Agent.
...
2. **Earl overlays** — transport type, auth patterns, aggregator flags,
...
# -- Earl overlay ----------------------------------------------------------
# Earl-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class EarlOverlay:
    """Earl-specific provider metadata layered on top of models.dev."""
...
EARL_OVERLAYS: Dict[str, EarlOverlay] = {
    "openrouter": EarlOverlay(
    ...
    "openai-codex": EarlOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": EarlOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": EarlOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="EARL_QWEN_BASE_URL",
    ),
    "google-gemini-cli": EarlOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": EarlOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": EarlOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": EarlOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": EarlOverlay(
    ...
    "minimax-oauth": EarlOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": EarlOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": EarlOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": EarlOverlay(
        transport="openai_chat",
    ...
    "opencode": EarlOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": EarlOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "earl", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Earl overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Earl overlay
    ...
    overlay = EARL_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + earl extra
    ...
    if overlay is not None:
        # Earl-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in EARL_OVERLAYS
    if provider == "bedrock":
