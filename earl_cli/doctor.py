"""
Doctor command for earl CLI.

Diagnoses issues with Earl Agent setup.
"""
...
from earl_cli.config import get_project_root, get_earl_home, get_env_path
from earl_cli.env_loader import load_earl_dotenv
from earl_constants import display_earl_home

PROJECT_ROOT = get_project_root()
EARL_HOME = get_earl_home()
_DHH = display_earl_home()  # user-facing display path (e.g. ~/.earl or ~/.earl/profiles/coder)

# Load environment variables from ~/.earl/.env so API key checks work
_env_path = get_env_path()
load_earl_dotenv(earl_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from earl_cli.colors import Colors, color
from earl_cli.models import _EARL_USER_AGENT
from earl_cli.vercel_auth import describe_vercel_auth
from earl_constants import OPENROUTER_MODELS_URL
...
from earl_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.earl/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("EARL_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("EARL_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from earl_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from earl_cli.auth import get_minimax_oauth_auth_status
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from earl_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `earl`.
    os.environ.setdefault("EARL_INTERACTIVE", "1")

    # Handle `earl doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.earl/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Earl Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`earl doctor --ack {hit.advisory.id}`."
                )
    ...
    # Check ~/.earl/.env (primary location for user config)
    env_path = EARL_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'earl setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'earl setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'earl setup' to create one")
                issues.append("Run 'earl setup' to create .env")
    
    # Check ~/.earl/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = EARL_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from earl_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from earl_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'earl config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from earl_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.earl/.env or run 'earl setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'earl setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'earl config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from earl_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from earl_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'earl setup' to migrate config")
                else:
                    issues.append("Run 'earl doctor --fix' or 'earl setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'earl doctor --fix'")
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Earl' own device-code flow — the Codex CLI is
    ...
    if earl_home.exists():
        ...
        earl_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = earl_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Earl a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Earl Agent Persona\n\n"
                "<!-- Edit this file to customize how Earl communicates. -->\n\n"
                "You are Earl, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = earl_home / "memories"
    ...
    state_db_path = earl_home / "state.db"
    ...
    wal_path = earl_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'earl doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "earl"
        ...
        _cmd_link = _cmd_link_dir / "earl"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(earl not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/earl → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/earl points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/earl → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/earl — run 'earl doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/earl exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/earl not found",
                    "(earl command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/earl → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/earl symlink — run 'earl doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'earl[vercel]')",
                "Install the Vercel optional dependency: pip install 'earl[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `earl doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'earl config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``earl doctor``.
    ...
    # never the legitimate source for `earl doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'earl setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = EARL_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: earl skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = EARL_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: earl memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: earl memory setup",
                    "No Honcho API key — run 'earl memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run earl memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: earl memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: earl memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from earl_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "earl -p" in content:
                            _m = _re.search(r"earl -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'earl doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
