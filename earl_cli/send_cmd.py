"""CLI subcommand: ``earl send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"earl send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"earl send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"earl send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"earl send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"earl send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `earl gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.earl/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_earl_env() -> None:
    """Populate ``os.environ`` from ``~/.earl/.env`` AND bridge top-level
    ...
    startup that ``earl send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.earl/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.earl/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``earl send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.earl/.env and ~/.earl/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "earl send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  earl send --to telegram \"hello\"\n"
            "  earl send --to discord:#ops --file report.md\n"
            "  earl send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "earl send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `earl send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Earl "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.earl/.env + ~/.earl/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  earl send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | earl send --to telegram:-1001234567890\n"
            "  earl send --to discord:#ops --file /tmp/report.md\n"
            "  earl send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  earl send --list                  # all platforms\n"
            "  earl send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `earl send --list telegram`.",
    )
