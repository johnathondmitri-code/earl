"""
Earl Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.earl/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Earl PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing earl or earl PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Earl Agent" comment and following line
                if '# Earl Agent' in line or '# earl' in line:
                    skip_next = True
                    continue
                if skip_next and ('earl' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing earl
                if 'earl' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the earl wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "earl",
        Path("/usr/local/bin/earl"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains earl_cli reference)
                content = wrapper.read_text()
                if 'earl_cli' in content or 'earl' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``earl gateway run`` processes
    ...
    try:
        from earl_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from earl_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``EARL_HOME`` and ``EARL_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\earl\git\cmd``, ``%LOCALAPPDATA%\earl\git\bin``,
#      ``%LOCALAPPDATA%\earl\git\usr\bin``, ``%LOCALAPPDATA%\earl\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\earl\git\`` and Node to
#      ``%LOCALAPPDATA%\earl\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``earl dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\earl\gateway-service\`` and sometimes
...
def _earl_path_markers(earl_home: Path) -> list[str]:
    """Path-entry substrings that identify Earl-owned User-PATH entries."""
    root = str(earl_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare earl install dir.
    markers = [root + "\\earl", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if EARL_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "earl".  We don't want to catch
    # unrelated entries like "cearl-foo" or "ephermeral", so we look for
    # backslash-earl as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(earl_home: Path) -> list[str]:
    """Strip Earl-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _earl_path_markers(earl_home)
            kept: list[str] = []
...
def remove_earl_env_vars_windows() -> list[str]:
    """Delete EARL_HOME and EARL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("EARL_HOME", "EARL_GIT_BASH_PATH"):
                try:
...
def _is_default_earl_home(earl_home: Path) -> bool:
    """Return True when ``earl_home`` points at the default (non-profile) root."""
    try:
        from earl_constants import get_default_earl_root
        return earl_home.resolve() == get_default_earl_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its EARL_HOME directory.

    We shell out to ``earl -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current EARL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m earl_cli.main` so we don't depend on a `earl`
    #    wrapper that may be half-removed mid-uninstall.
    earl_invocation = [_sys.executable, "-m", "earl_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                earl_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's EARL_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.earl/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.earl/ for future reinstall
    """
    project_root = get_project_root()
    earl_home = get_earl_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie EARL_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_earl_home(earl_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Earl Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {earl_home / 'config.yaml'}")
    print(f"  Secrets: {earl_home / '.env'}")
    print(f"  Data:    {earl_home / 'cron/'}, {earl_home / 'sessions/'}, {earl_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their EARL_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Earl data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Earl code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in earl_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\earl\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(earl_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Earl-owned PATH entries in User environment")

        log_info("Removing EARL_HOME / EARL_GIT_BASH_PATH User env vars...")
        removed_env = remove_earl_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Earl-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing earl command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.earl/, just remove the earl subdir
            if earl_home in project_root.parents or project_root.parent == earl_home:
    ...
    #     under EARL_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(earl_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(earl_home)
    ...
    # 5. Optionally remove ~/.earl/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile EARL_HOME dirs live under
        ...
        try:
            if earl_home.exists():
                shutil.rmtree(earl_home)
                log_success(f"Removed {earl_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {earl_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {earl_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/earl/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/earl/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Earl Agent! ⚕")
    print()
