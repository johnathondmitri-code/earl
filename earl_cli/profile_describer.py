...
Used by ``earl profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``earl_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Earl Agent kanban board.
...
Rules:
  ...
  - Never write "Earl Agent profile" or other meta-narration.
...
def describe_profile(
    ...
    *,
    ...
    try:
        if canon == "default":
            from earl_constants import get_earl_home  # type: ignore
            profile_dir = Path(get_earl_home())
        else:
