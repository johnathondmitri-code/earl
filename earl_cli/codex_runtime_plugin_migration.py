"""Migrate Earl' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Earl' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Earl mcp_servers.<n>.command/args/env  → codex stdio transport
  Earl mcp_servers.<n>.url/headers       → codex streamable_http transport
  Earl mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Earl mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Earl-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Earl' own
...
MIGRATION_MARKER = (
    "# managed by earl — `earl codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end earl managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Earl config.")
...
# Earl keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Earl' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, earl_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Earl MCP server config to the codex inline-table dict
    ...
    if not isinstance(earl_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(earl_cfg.get("command"))
    has_url = bool(earl_cfg.get("url"))
    ...
    for key in earl_cfg:
        ...
        elif key not in _KNOWN_EARL_KEYS:
            skipped.append(f"{key} (unknown Earl key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (EARL_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Earl)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Earl' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Earl' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Earl
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="earl-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a EARL_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed earl-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.earl`` symlink that happens to live under ``/private/var/folders``
...
def _build_earl_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Earl' own
    ...
    so a earl installed under /opt/, /usr/local/, or a venv all work.
    EARL_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # EARL_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_earl_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever EARL_HOME its launcher sets
    ...
    # — that would override the launcher's EARL_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("EARL_HOME", tmp_path) would
    ...
    earl_home = os.environ.get("EARL_HOME") or ""
    if earl_home and _looks_like_test_tempdir(earl_home):
        earl_home = ""
    if earl_home:
        env["EARL_HOME"] = earl_home
    # PYTHONPATH passes through so a worktree-launched earl finds the
    ...
    env["EARL_QUIET"] = "1"
    env["EARL_REDACT_SECRETS"] = env.get("EARL_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.earl_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_earl_tools: bool = True,
) -> MigrationReport:
    """Translate Earl mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        earl_config: full ~/.earl/config.yaml dict
        ...
        expose_earl_tools: when True (default), register Earl' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Earl for tools
    ...
    earl_servers = (earl_config or {}).get("mcp_servers") or {}
    if not isinstance(earl_servers, dict):
        report.errors.append(
            "mcp_servers in Earl config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in earl_servers.items():
    ...
    # Inject Earl' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Earl for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/earl_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_earl_tools:
        translated["earl-tools"] = _build_earl_tools_mcp_entry()
        if "earl-tools" not in report.migrated:
            report.migrated.append("earl-tools")
