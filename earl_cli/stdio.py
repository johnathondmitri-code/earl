...
on Japanese locales, etc.).  Earl's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``earl_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Earl is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``EARL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("EARL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Earl-managed Git install directories so
    ...
    # already-running shells don't see the change — which means earl
    ...
    # install without Earl-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Earl's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Earl's
    ``earl config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Earl (User env var in Windows Settings, or
    export in a PowerShell profile) and Earl picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Earl-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\earl\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``earl`` from right after install) retain their old PATH.

    Any subprocess Earl spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Earl-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "earl", "git", "cmd"),
        os.path.join(local_appdata, "earl", "git", "bin"),
        os.path.join(local_appdata, "earl", "git", "usr", "bin"),
        # Earl venv Scripts directory — host of the earl.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes earl, but harmless to include.
        os.path.join(local_appdata, "earl", "earl", "venv", "Scripts"),
