"""Profile distributions — shareable, packaged Earl profiles via git.

A distribution is a Earl profile published as a git repository (or
...
* ``earl profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``earl skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``earl profile``, not a parallel tree):

    earl profile install <source> [--name N] [--alias] [--force] [--yes]
    earl profile update  <name>  [--force-config] [--yes]
    earl profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "earl", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            earl_requires=str(data.get("earl_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.earl_requires:
            out["earl_requires"] = self.earl_requires
        if self.author:
...
def check_earl_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Earl {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Earl distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``earl profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Earl profile distribution."
            )
...
def plan_install(
    ...
    from earl_cli import __version__ as earl_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Earl distribution."
        )

    # Version check up-front so we fail fast
    check_earl_requires(manifest.earl_requires, earl_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.earl).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="earl_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `earl profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`earl profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="earl_dist_update_") as tmp:
        plan = plan_install(
