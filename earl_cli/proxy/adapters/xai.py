...
# xAI's public API is OpenAI-compatible for the endpoints Earl commonly
# uses. The Responses endpoint is included because Earl' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Earl-managed OAuth credentials."""

    auth_hint = "earl auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`earl auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`earl auth reset xai-oauth` or re-authenticate with "
                    "`earl auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `earl auth add xai-oauth --type oauth`."
            )
