"""
Profile management for multiple isolated Earl instances.

Each profile is a fully independent EARL_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.earl/profiles/<name>/`` by default.

The "default" profile is ``~/.earl`` itself — backward compatible,
zero migration needed.

Usage::

    earl profile create coder          # fresh profile + bundled skills
    earl profile create coder --clone  # also copy config, .env, SOUL.md, skills
    earl profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    earl -p coder chat                 # or via flag
    earl profile use coder             # set as sticky default
    earl profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See earl_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `earl profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `earl
...
# `earl skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Earl infrastructure directories that only the default profile
         (``~/.earl``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_earl_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.earl) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "earl",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "earl", "default", "test", "tmp", "root", "sudo",
})

# Earl subcommands that cannot be used as profile names/aliases
_EARL_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the earl root, NOT to the current EARL_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where EARL_HOME points outside
    ``~/.earl``, profiles live under ``EARL_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_earl_home() / "profiles"


def _get_default_earl_home() -> Path:
    """Return the default (pre-profile) EARL_HOME path.

    In standard deployments this is ``~/.earl``.
    In Docker/custom deployments where EARL_HOME is outside ``~/.earl``
    (e.g. ``/opt/data``), returns EARL_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_earl_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Earl installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its EARL_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_earl_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, earl subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _EARL_SUBCOMMANDS:
        return f"'{canon}' conflicts with a earl subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``earl profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Earl config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_earl_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``earl update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.earl)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.earl/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `earl update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`earl profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `earl update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `earl profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches EARL_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``earl profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "EARL_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.earl).\n"
            "To remove everything, use: earl uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set EARL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("EARL_HOME")
    try:
        os.environ["EARL_HOME"] = str(profile_dir)
        from earl_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["EARL_HOME"] = old_home
        elif "EARL_HOME" in os.environ:
            del os.environ["EARL_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.earl/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: earl profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from EARL_HOME.

    Returns ``"default"`` if EARL_HOME is not set or points to ``~/.earl``.
    Returns the profile name if EARL_HOME points into ``~/.earl/profiles/<name>``.
    Returns ``"custom"`` if EARL_HOME is set to an unrecognized path.
    """
    from earl_constants import get_earl_home
    earl_home = get_earl_home()
    resolved = earl_home.resolve()

    default_resolved = _get_default_earl_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.earl itself — its parent is ~/ and its
        # directory name is ".earl", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: earl profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.earl itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.earl). "
            "Specify a different name: earl profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="earl_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"earl.{old_name}"
    new_host = f"earl.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_earl_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a EARL_HOME path string.

    Called early in the CLI entry point, before any earl modules
    are imported, to set the EARL_HOME environment variable.
    """
