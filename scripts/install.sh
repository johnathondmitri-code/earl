...
# Python-driven tool session (e.g. Earl terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/earl.git"
REPO_URL_HTTPS="https://github.com/NousResearch/earl.git"
EARL_HOME="${EARL_HOME:-$HOME/.earl}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${EARL_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$EARL_INSTALL_DIR"
...
#   code at /usr/local/lib/earl, command at /usr/local/bin/earl,
#   data still at /root/.earl (EARL_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.earl/earl"
            echo "                   default (root, Linux): /usr/local/lib/earl"
            echo "  --earl-home PATH  Data directory (default: ~/.earl, or \$EARL_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Earl installs the code under"
            echo "  /usr/local/lib/earl and links the command into"
            echo "  /usr/local/bin/earl (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$EARL_HOME"
            echo "  (default /root/.earl).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$EARL_HOME/earl are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs earl setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `earl` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $EARL_HOME/earl
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $EARL_HOME/earl
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/earl
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $EARL_HOME/earl — then preserve it)
#
# Always no-op when the user set --dir or $EARL_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in EARL_HOME.
    if is_termux; then
        INSTALL_DIR="$EARL_HOME/earl"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$EARL_HOME/earl/.git" ]; then
            INSTALL_DIR="$EARL_HOME/earl"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/earl)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/earl"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/earl"
        log_info "  Data:    $EARL_HOME (unchanged)"
        return 0
    fi
...
get_earl_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/earl" ]; then
        echo "$link_dir/earl"
    else
        echo "earl"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/NousResearch/earl/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/earl-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/earl-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$EARL_HOME/node/bin/node" ]; then
        export PATH="$EARL_HOME/node/bin:$PATH"
        local found_ver=$("$EARL_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Earl-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.earl/node/..."
    ...
    # Place into ~/.earl/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$EARL_HOME/node"
    mkdir -p "$EARL_HOME"
    mv "$extracted_dir" "$EARL_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$EARL_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$EARL_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$EARL_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$EARL_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$EARL_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.earl/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Earl install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Earl Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="earl-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Earl behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up earl command..."

    if [ "$USE_VENV" = true ]; then
        EARL_BIN="$INSTALL_DIR/venv/bin/earl"
    else
        EARL_BIN="$(which earl 2>/dev/null || echo "")"
        if [ -z "$EARL_BIN" ]; then
            log_warn "earl not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$EARL_BIN" ]; then
        log_warn "earl entry point not found at $EARL_BIN"
    ...
    # Create a user-facing shim for the earl command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $EARL_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$EARL_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/earl"
    cat > "$command_link_dir/earl" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$EARL_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/earl"
    log_success "Installed earl launcher → $command_link_display_dir/earl"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "earl command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v earl' \
                >/dev/null 2>&1; then
        ...
        log_info "earl not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Earl Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Earl Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Earl Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so earl works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.earl directory structure (config at top level, code in subdir)
    mkdir -p "$EARL_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.earl/.env (top level, easy to find)
    if [ ! -f "$EARL_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$EARL_HOME/.env"
            log_success "Created ~/.earl/.env from template"
        else
            touch "$EARL_HOME/.env"
            log_success "Created ~/.earl/.env"
        fi
    else
        log_info "~/.earl/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$EARL_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.earl/config.yaml (top level, easy to find)
    if [ ! -f "$EARL_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$EARL_HOME/config.yaml"
            log_success "Created ~/.earl/config.yaml from template"
        fi
    else
        log_info "~/.earl/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$EARL_HOME/SOUL.md" ]; then
        cat > "$EARL_HOME/SOUL.md" << 'SOUL_EOF'
# Earl Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Earl communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.earl/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.earl/"

    # Seed bundled skills into ~/.earl/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.earl/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.earl/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$EARL_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$EARL_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.earl/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Earl Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Earl will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (earl --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'earl setup' after install."
        return 0
    fi
    ...
    # Run earl setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m earl_cli.main setup < /dev/tty
    else
        python -m earl_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$EARL_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Earl to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$EARL_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'earl whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                EARL_CMD="$(get_earl_command_path)"
                $EARL_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'earl whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'earl gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $EARL_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $EARL_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: earl gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: earl gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $EARL_CMD gateway > "$EARL_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.earl/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: earl gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: earl gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $EARL_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $EARL_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $EARL_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}earl${NC}              Start chatting"
    echo -e "   ${GREEN}earl setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}earl config${NC}       View/edit configuration"
    echo -e "   ${GREEN}earl config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}earl gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}earl update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'earl' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'earl' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'earl' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$EARL_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$EARL_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$EARL_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$EARL_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$EARL_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Earl for pip install"
    ...
    EARL_CMD="$(command -v earl 2>/dev/null || echo "")"
    if [ -n "$EARL_CMD" ]; then
        log_info "Running earl setup..."
        "$EARL_CMD" setup
    else
        log_warn "earl command not found on PATH"
        log_info "Try: python -m earl_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$EARL_HOME/.install_method"
}
