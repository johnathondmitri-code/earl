#!/usr/bin/env python3
"""Build the Earl Model Catalog — a centralized JSON manifest of curated models.
...
Earl CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Earl release.
...
``https:/.earl-agent.nousresearch.com/docs/api/model-catalog.json``
"""
...
# Ensure EARL_HOME is set for imports that touch it at module level.
os.environ.setdefault("EARL_HOME", os.path.join(os.path.expanduser("~"), ".earl"))

from earl_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "earl repo",
            "docs": "https:/.earl-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
