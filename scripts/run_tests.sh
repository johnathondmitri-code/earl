#!/usr/bin/env bash
# Canonical test runner for earl. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.earl/earl/venv"; do
...
# Unset EARL_* behavioral vars too.
unset EARL_YOLO_MODE EARL_INTERACTIVE EARL_QUIET EARL_TOOL_PROGRESS \
      EARL_TOOL_PROGRESS_MODE EARL_MAX_ITERATIONS EARL_SESSION_PLATFORM \
      EARL_SESSION_CHAT_ID EARL_SESSION_CHAT_NAME EARL_SESSION_THREAD_ID \
      EARL_SESSION_SOURCE EARL_SESSION_KEY EARL_GATEWAY_SESSION \
      EARL_CRON_SESSION \
      EARL_PLATFORM EARL_INFERENCE_PROVIDER EARL_MANAGED EARL_DEV \
      EARL_CONTAINER EARL_EPHEMERAL_SYSTEM_PROMPT EARL_TIMEZONE \
      EARL_REDACT_SECRETS EARL_BACKGROUND_NOTIFICATIONS EARL_EXEC_ASK \
      EARL_HOME_MODE 2>/dev/null || true
...
# If a system-wide earl pytest_live_guard plugin is installed at
# $HOME/.earl/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.earl/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.earl:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.earl" ;;
  esac
