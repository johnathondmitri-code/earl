#!/usr/bin/env python3
"""Drive the Earl TUI under EARL_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.earl/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with EARL_TUI_RESUME set so it
bypasses the earl_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  EARL_PERF_LOG     (default ~/.earl/perf.log)
  EARL_PERF_NODE    (default node from $PATH)
  EARL_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests EARL_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_earl_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("EARL_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".earl"

DEFAULT_TUI_DIR = Path(
    os.environ.get("EARL_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("EARL_PERF_LOG", str(get_earl_home() / "perf.log")))
DEFAULT_STATE_DB = get_earl_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — EARL_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["EARL_DEV_PERF"] = "1"
    env["EARL_DEV_PERF_MS"] = str(args.threshold_ms)
    env["EARL_DEV_PERF_LOG"] = str(log)
    env["EARL_TUI_RESUME"] = sid
    ...
    node = os.environ.get("EARL_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="EARL_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "earl-ink" / "src"
