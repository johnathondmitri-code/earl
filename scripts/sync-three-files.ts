/**
 * Upload run_agent.py, toolsets.py, AND tools/pipedream_tool.py into the
 * live sandbox. Used to push debugging tracer lines without a git pull.
 */
...
async function main() {
  ...
  if (!sandboxId) {
    console.error("Usage: sync-three-files.ts <sandboxId>");
    process.exit(2);
  }
