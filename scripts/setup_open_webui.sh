...
# Bootstrap Open WebUI against Earl Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.earl/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-earl.sh
...
#   EARL_API_PORT=8642
#   EARL_API_HOST=127.0.0.1
#   EARL_API_MODEL_NAME='Earl Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Earl Agent WebUI}"
...
EARL_ENV_FILE="${EARL_ENV_FILE:-$HOME/.earl/.env}"
EARL_API_PORT="${EARL_API_PORT:-8642}"
EARL_API_HOST="${EARL_API_HOST:-127.0.0.1}"
EARL_API_CONNECT_HOST="${EARL_API_CONNECT_HOST:-127.0.0.1}"
EARL_API_MODEL_NAME="${EARL_API_MODEL_NAME:-Earl Agent}"
EARL_API_BASE_URL="http://${EARL_API_CONNECT_HOST}:${EARL_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-earl.sh"
LOG_DIR="$HOME/.earl/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$EARL_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$EARL_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.earl.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.earl</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.earl"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.earl"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-earl.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Earl Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-earl.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.earl/logs/openwebui.log
StandardError=append:%h/.earl/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-earl.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$EARL_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Earl API server is configured...'
  upsert_env API_SERVER_ENABLED true "$EARL_ENV_FILE"
  upsert_env API_SERVER_HOST "$EARL_API_HOST" "$EARL_ENV_FILE"
  upsert_env API_SERVER_PORT "$EARL_API_PORT" "$EARL_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$EARL_API_MODEL_NAME" "$EARL_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$EARL_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Earl gateway so API server settings take effect...'
  earl gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${EARL_API_CONNECT_HOST}:${EARL_API_PORT}/health" >/dev/null; then
    log 'Earl API server did not answer on the first check. Trying to start gateway in the background...'
    nohup earl gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${EARL_API_CONNECT_HOST}:${EARL_API_PORT}/health" >/dev/null
  ...
  log "Earl API endpoint: ${EARL_API_BASE_URL}"
