"""Windows UTF-8 bootstrap for Earl entry points.
...
should be imported at the very top of every Earl entry point
(``earl``, ``earl``, ``earl-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import earl_bootstrap``
# (or ``from earl_bootstrap import apply_windows_utf8_bootstrap``) at
