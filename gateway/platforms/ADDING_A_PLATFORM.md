...
There are two ways to add a platform to the Earl gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.earl/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Earl code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `earl gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `earl_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Earl core.
...
And add it to the `earl-gateway` composite:

```python
"earl-gateway": {
    "includes": [..., "earl-your-platform"]
}
```
...
## 12. Status Display (`earl_cli/status.py`)
...
## 13. Gateway Setup Wizard (`earl_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/.earl_cli/ toolsets.py \
  --include="*.py" -l | sort -u
