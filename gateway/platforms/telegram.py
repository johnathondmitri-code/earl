...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("EARL_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "EARL_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "EARL_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Earl-created private-chat topic lanes are marked with
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `earl update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Earl or OpenClaw instance is running "
            "with this token, then restart the gateway with 'earl gateway restart'."
    ...
    def _persist_dm_topic_thread_id(self, chat_id: int, topic_name: str, thread_id: int) -> None:
        ...
        try:
            ...
            config_path = get_earl_home() / "config.yaml"
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Earl process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("EARL_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("EARL_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("EARL_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("EARL_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("EARL_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("EARL_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``earl update --gateway``
    ...
    # Scripts live in ~/.earl/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".earl" / "scripts" / "gmail-triage" / script_name
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@earl_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Earl bot profiles. A message like
