...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_earl_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.earl/config.yaml (primary user-facing config)
    3. ~/.earl/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_earl_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``earl config set gateway.streaming.*``
