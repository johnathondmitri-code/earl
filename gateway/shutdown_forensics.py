...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        earl_home_str = os.environ.get("EARL_HOME")
        if earl_home_str:
            takeover_path = Path(earl_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(earl_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded earl but never re-ran ``earl setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../earl-gateway.service"
    ...
    # that's the common case for earl.
    ...
    # constant in earl_cli/gateway.py.
    headroom = 30.0
