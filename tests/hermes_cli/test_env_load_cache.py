...
The cache exists to keep `earl tools` → "All Platforms" fast: every
...
def test_load_env_caches_on_repeat_calls():
    ...
    from earl_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("earl_cli.config.get_env_path", return_value=env_path):
            first = load_env()
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from earl_cli import config as config_mod
    from earl_cli.config import invalidate_env_cache, load_env, save_env_value
...
def test_load_env_handles_missing_file():
    ...
    try:
        with patch("earl_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
