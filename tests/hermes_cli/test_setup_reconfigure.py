...
- Bare `earl setup` drops straight into the full reconfigure wizard
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("earl_cli.setup.ensure_hermes_home", {}),
        ("earl_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("earl_cli.config.is_managed", {"return_value": False}),
        ("earl_cli.setup.load_config", {"return_value": {}}),
        ("earl_cli.setup.save_config", {}),
        ("earl_cli.setup.get_env_value", {"return_value": None}),
        ("earl_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("earl_cli.setup._print_setup_summary", {}),
        ("earl_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("earl_cli.setup.load_config", {"return_value": {}}),
        ("earl_cli.setup.save_config", {}),
        ("earl_cli.auth.get_active_provider", {"return_value": None}),
        ("earl_cli.setup.get_env_value", {"return_value": None}),
        ("earl_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `earl setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="earl_cli.setup.prompt_choice",
                quick="earl_cli.setup._run_quick_setup",
                model="earl_cli.setup.setup_model_provider",
                terminal="earl_cli.setup.setup_terminal_backend",
                agent="earl_cli.setup.setup_agent_settings",
                gateway="earl_cli.setup.setup_gateway",
                tools="earl_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`hermes setup --reconfigure` behaves the same as bare `earl setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("earl_cli.setup.prompt_choice", {"return_value": 0}),
                first="earl_cli.setup._run_first_time_quick_setup",
            )
            from earl_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from earl_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "earl_cli.setup.run_setup_wizard",
