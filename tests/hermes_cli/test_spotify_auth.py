...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from earl_cli.auth_commands import auth_status_command
...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes EARL_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("EARL_HOME", str(tmp_path))
    ...
    assert "EARL_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "EARL_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "EARL_SPOTIFY_CLIENT_ID" not in env_path.read_text()
