"""Tests for --ignore-user-config and --ignore-rules flags on `earl chat`.
...
* ``--ignore-user-config`` → skip ``~/.earl/config.yaml`` in ``load_cli_config()``
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("EARL_IGNORE_USER_CONFIG", "EARL_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``EARL_IGNORE_USER_CONFIG=1``.
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("EARL_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting EARL_IGNORE_RULES=1 flips HermesCLI.ignore_rules True."""
        monkeypatch.setenv("EARL_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("EARL_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("EARL_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.HermesCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``earl_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("EARL_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("EARL_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("EARL_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("EARL_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("EARL_IGNORE_USER_CONFIG") == "1"
        assert "EARL_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "EARL_IGNORE_USER_CONFIG" not in os.environ
        assert "EARL_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        ...
        import inspect
        import earl_cli.main as hm
        src = inspect.getsource(hm)
        assert "EARL_IGNORE_USER_CONFIG" in src
        assert "EARL_IGNORE_RULES" in src
