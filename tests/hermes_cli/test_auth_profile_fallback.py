...
When ``EARL_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.earl/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.earl            (has its own auth.json fixture)
    * Profile     -> tmp_path/.earl/profiles/coder   (active, EARL_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (EARL_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.earl as the "real home" — which is NOT equal
    # to the EARL_HOME we set (tmp_path/classic), so the guard passes.
    ...
    from earl_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: EARL_HOME is set to a custom path that is NOT under
    # ~/.earl/profiles/ — get_default_hermes_root() returns EARL_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from earl_cli.auth import read_credential_pool, write_credential_pool
