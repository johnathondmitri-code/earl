...
Bug 3: `earl model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.earl/.env but no valid
...
from earl_cli.config import load_env, save_env_value
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        from earl_cli.main import _model_flow_anthropic
        cfg = {}
