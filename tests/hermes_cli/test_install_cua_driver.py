...
* Be macOS-only — no-op silently on Linux/Windows so ``earl update`` can
...
class TestInstallCuaDriverUpgrade:
    def test_upgrade_on_non_macos_is_silent_noop(self):
        """``earl update`` calls install_cua_driver(upgrade=True) for every
        ...
        from earl_cli import tools_config
    ...
    def test_upgrade_on_macos_with_binary_runs_installer(self):
        ...
        with patch("platform.system", return_value="Darwin"), \
            ...
            # "grant macOS permissions" block on every `earl update`.
    ...
    def test_non_upgrade_on_macos_with_binary_skips_install(self):
        ...
        This is the behaviour that ``earl tools`` (re)enable depends on,
