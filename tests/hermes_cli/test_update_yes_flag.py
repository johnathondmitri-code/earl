...
class TestUpdateYesConfigMigration:
    ...
    @patch("earl_cli.config.migrate_config")
    @patch("earl_cli.config.check_config_version", return_value=(1, 2))
    @patch("earl_cli.config.get_missing_config_fields", return_value=[])
    @patch("earl_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``earl_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``earl_cli.main`` first could leave
