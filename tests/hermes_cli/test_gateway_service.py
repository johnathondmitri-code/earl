...
import earl_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`earl gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `earl gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `earl gateway restart`)."""
        unit_path = tmp_path / "hermes-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``EARL_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` EARL_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir EARL_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="EARL_HOME=/tmp/pytest-of-alice/pytest-42/'
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_hermes_home_for_target_user", lambda home: "/home/alice/.earl")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("EARL_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("EARL_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("EARL_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.earl.gateway.plist"
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.earl.gateway"])
            ),
        )
...
class TestSystemUnitHermesHome:
    """EARL_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'EARL_HOME=/home/alice/.earl' in unit
        assert '/root/.earl' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: EARL_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("EARL_HOME", "/root/.earl/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'EARL_HOME=/home/alice/.earl/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_hermes_home(self, monkeypatch):
        # Custom EARL_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("EARL_HOME", "/opt/hermes-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'EARL_HOME=/opt/hermes-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's EARL_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hermes_home = str(gateway_cli.get_hermes_home().resolve())
        assert f'EARL_HOME={hermes_home}' in unit
...
class TestHermesHomeForTargetUser:
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._hermes_home_for_target_user("/home/alice")
        assert result == "/home/alice/.earl/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("EARL_HOME", "/opt/hermes")
...
class TestProfileArg:
    ...
    def test_default_hermes_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.earl should not produce a --profile flag."""
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.earl/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".earl" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("EARL_HOME", str(tmp_path / ".earl"))
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile EARL_HOME should return empty string."""
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.earl/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".earl" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".earl" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("EARL_HOME", str(profile_dir))
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".earl" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.earl.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".earl" / "earl"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".earl" / "earl")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".earl" / "earl")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("EARL_HOME", str(root_home / ".earl"))
        monkeypatch.setattr(gateway_cli, "get_hermes_home", lambda: root_home / ".earl")
        ...
        assert "WorkingDirectory=/home/alice/.earl/earl" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "earl-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "earl-gateway"])
...
class TestLegacyHermesUnitDetection:
    ...
    older install left a ``earl.service`` unit behind when the service was
    renamed to ``earl-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``earl.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``earl-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Hermes Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m earl_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_hermes_service_in_system_scope(self, tmp_path, monkeypatch):
        ...
        legacy = system_dir / "earl.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_hermes_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — ``earl-gateway-coder.service`` is our standard
    ...
    def test_ignores_unrelated_hermes_service(self, tmp_path, monkeypatch):
        """Third-party ``earl.service`` that isn't ours stays untouched.

        If a user has some other package named ``earl`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "earl.service").write_text(
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "earl.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "earl.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m earl_cli.main gateway run
          - python path/to/earl_cli/main.py gateway run
          - hermes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m earl_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/earl_cli/main.py gateway run",
        ...
        for i, execstart in enumerate(variants):
            name = f"earl.service" if i == 0 else f"earl.service"  # same name
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "earl.service"
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "earl.service"
        system_legacy = system_dir / "earl.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `earl gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import earl_cli.main as cli_main
        ...
        # public, just confirm that `earl gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import earl_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "earl_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "earl_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``earl gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "earl-gateway")
...
class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``earl gateway start --system`` etc.) must
