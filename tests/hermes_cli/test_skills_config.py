"""Tests for earl_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("earl_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from earl_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("earl_cli.config.load_config")
    ...
    @patch("earl_cli.config.load_config")
    @patch.dict("os.environ", {"EARL_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """EARL_SESSION_PLATFORM should be used when EARL_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_hermes_platform_takes_precedence(self, tmp_path, monkeypatch):
        """EARL_PLATFORM should win over EARL_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("EARL_HOME", str(tmp_path))
        monkeypatch.setenv("EARL_PLATFORM", "telegram")
        monkeypatch.setenv("EARL_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from earl_cli.skills_config import _get_categories
        skills = [
