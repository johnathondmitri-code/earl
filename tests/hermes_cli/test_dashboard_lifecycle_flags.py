"""Tests for ``earl dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``earl update``
...
from earl_cli.main import cmd_dashboard, _report_dashboard_status
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("earl_cli.main._find_stale_dashboard_pids",
             ...
             patch("earl_cli.main._kill_stale_dashboard_processes") as mock_kill, \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("earl_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("earl_cli.main._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``earl dashboard --stop`` must not end up ALSO starting
    a new server."""
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("earl_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"earl_cli.web_server": fake_ws}), \
             pytest.raises(SystemExit):
            cmd_dashboard(_ns(stop=True))
...
class TestArgparseWiring:
    ...
    ``earl dashboard --stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from earl_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("earl_cli.main")
