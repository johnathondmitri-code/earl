"""Unit tests for earl_cli/dingtalk_auth.py (QR device-flow registration)."""
...
class TestApiPost:
    ...
    def test_raises_on_network_error(self):
        import requests
        from earl_cli.dingtalk_auth import _api_post, RegistrationError

        with patch("earl_cli.dingtalk_auth.requests.post",
    ...
    def test_raises_on_nonzero_errcode(self):
        ...
        with patch("earl_cli.dingtalk_auth.requests.post", return_value=mock_resp):
            with pytest.raises(RegistrationError, match=r"boom \(errcode=42\)"):
                _api_post("/app/registration/init", {"source": "hermes"})

    def test_returns_data_on_success(self):
        from earl_cli.dingtalk_auth import _api_post
...
class TestBeginRegistration:
    ...
    def test_chains_init_then_begin(self):
        from earl_cli.dingtalk_auth import begin_registration
        ...
        with patch("earl_cli.dingtalk_auth._api_post", side_effect=responses):
    ...
    def test_missing_nonce_raises(self):
        from earl_cli.dingtalk_auth import begin_registration, RegistrationError

        with patch("earl_cli.dingtalk_auth._api_post",
...
class TestWaitForSuccess:
    ...
    def test_returns_credentials_on_success(self):
        from earl_cli.dingtalk_auth import wait_for_registration_success
        ...
        with patch("earl_cli.dingtalk_auth.poll_registration", side_effect=responses), \
             patch("earl_cli.dingtalk_auth.time.sleep"):
    ...
    def test_success_without_credentials_raises(self):
        from earl_cli.dingtalk_auth import wait_for_registration_success, RegistrationError

        with patch("earl_cli.dingtalk_auth.poll_registration",
...
class TestRenderQR:
    ...
    def test_returns_false_when_qrcode_missing(self, monkeypatch):
        from earl_cli import dingtalk_auth
    ...
    def test_prints_when_qrcode_available(self, capsys):
        ...
        from earl_cli.dingtalk_auth import render_qr_to_terminal
...
class TestConfigOverrides:
    ...
    def test_base_url_default(self, monkeypatch):
        ...
        import importlib
        import earl_cli.dingtalk_auth as mod
