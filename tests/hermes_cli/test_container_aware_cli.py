...
def container_env(tmp_path, monkeypatch):
    """Set up a fake EARL_HOME with .container-mode file."""
    hermes_home = tmp_path / ".earl"
    hermes_home.mkdir()
    monkeypatch.setenv("EARL_HOME", str(hermes_home))
    monkeypatch.delenv("EARL_DEV", raising=False)
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("earl_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "earl"
...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("earl_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_hermes_dev(container_env, monkeypatch):
    """Returns None when EARL_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("EARL_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_hermes_dev_zero(container_env, monkeypatch):
    """EARL_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("EARL_DEV", "0")
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from earl_cli.main import _exec_in_container
