"""Tests for the Kanban DB layer (earl_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated EARL_HOME with an empty kanban DB."""
    home = tmp_path / ".earl"
    home.mkdir()
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("EARL_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import earl_cli.kanban_db as _kb
...
class TestSharedBoardPaths:
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.earl/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.earl/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.earl/kanban.db.
        default_home = tmp_path / ".earl"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # EARL_HOME and confirm equality. This is the property the
        ...
        monkeypatch.setenv("EARL_HOME", str(profile_home))
    ...
    def test_docker_custom_hermes_home_uses_env_path_directly(
        ...
        # Docker / custom deployment: EARL_HOME points outside ~/.earl.
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: EARL_HOME=/opt/hermes/profiles/coder;
    ...
    def test_explicit_override_via_hermes_kanban_home(
        ...
        # Explicit override: EARL_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".earl"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("EARL_HOME", str(profile_home))
        monkeypatch.setenv("EARL_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("EARL_HOME", str(default_home))
        monkeypatch.setenv("EARL_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # EARL_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile EARL_HOME and reads.
        monkeypatch.setenv("EARL_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_hermes_kanban_db_pin_beats_kanban_home(
        ...
        # EARL_KANBAN_DB pins the file path directly and beats both
        # EARL_KANBAN_HOME and the `get_default_hermes_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("EARL_HOME", str(default_home))
        monkeypatch.setenv("EARL_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("EARL_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows EARL_KANBAN_HOME -- the pins
        # are independent.
    ...
    def test_hermes_kanban_workspaces_root_pin_beats_kanban_home(
        ...
        # EARL_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".earl"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("EARL_HOME", str(default_home))
        monkeypatch.setenv("EARL_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("EARL_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows EARL_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("EARL_HOME", str(default_home))
        monkeypatch.setenv("EARL_KANBAN_DB", "   ")
        monkeypatch.setenv("EARL_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject EARL_KANBAN_DB
        # and EARL_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites EARL_HOME.
        default_home = tmp_path / ".earl"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["EARL_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["EARL_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["EARL_KANBAN_TASK"] == "t_dispatch_env"
        assert env["EARL_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see earl_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("earl_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="earl_state"):
            conn = kb.connect()
...
def test_resolve_hermes_argv_prefers_path_shim(monkeypatch):
    ...
    import shutil
    import earl_cli.kanban_db as kb

    monkeypatch.delenv("EARL_BIN", raising=False)
...
def test_resolve_hermes_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("EARL_BIN", ".\\hermes.exe")
...
def test_resolve_hermes_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "earl.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("EARL_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_hermes_argv() == [sys.executable, "-m", "earl_cli.main"]


def test_resolve_hermes_argv_honors_hermes_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like EARL_BIN lets service managers pin the executable."""
    import shutil
    import earl_cli.kanban_db as kb
    ...
    monkeypatch.setenv("EARL_BIN", str(shim))
...
def test_resolve_hermes_argv_hermes_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare EARL_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import earl_cli.kanban_db as kb
    ...
    monkeypatch.setenv("EARL_BIN", "hermes")
...
def test_resolve_hermes_argv_hermes_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare EARL_BIN does not accept current-directory shadow executables."""
    import sys
    import earl_cli.kanban_db as kb

    (tmp_path / "earl.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_hermes_argv_hermes_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved EARL_BIN batch shim is not used as worker argv[0]."""
    import sys
    import earl_cli.kanban_db as kb
...
def test_resolve_hermes_argv_hermes_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved EARL_BIN command names do not delegate cwd search to Popen."""
    import sys
    import earl_cli.kanban_db as kb
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m earl_cli.main`.
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_hermes_argv()
    assert argv == [sys.executable, "-m", "earl_cli.main"]
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `earl_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m earl_cli.main --version`
