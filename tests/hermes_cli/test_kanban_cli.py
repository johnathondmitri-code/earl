"""Tests for the kanban CLI surface (earl_cli.kanban)."""
...
from earl_cli import kanban as kc
...
def test_kanban_in_autocomplete_table():
    from earl_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_not_gateway_only():
    ...
    from earl_cli.commands import COMMAND_REGISTRY
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("EARL_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("EARL_KANBAN_BOARD") == "beta"
