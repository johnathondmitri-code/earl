...
normal tests assume away:
  ...
  - Filesystem: EARL_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own EARL_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            os.environ["EARL_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("earl_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from earl_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "EARL_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from earl_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["EARL_HOME"]).resolve())
...
def _(home, kb):
    """EARL_HOME at a path with spaces — should work but catches
    ...
    try:
        ...
        print(f"  EARL_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    """EARL_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  EARL_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    """EARL_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same EARL_HOME share DB correctly")
    finally:
...
def _idempotency_race_worker(hermes_home: str, key: str, result_file: str,
    ...
    os.environ["EARL_HOME"] = hermes_home
...
def _(home, kb):
    ...
    try:
        from earl_cli import web_server as ws  # noqa
    except Exception:
        pass
