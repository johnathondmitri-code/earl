...
Without this fallback, users on NFS-mounted ``EARL_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    earl_state._set_last_init_error(None)
    yield
    earl_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    earl_state._wal_fallback_warned_paths.clear()
    yield
    earl_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see earl_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        earl_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("earl_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        earl_state._set_last_init_error("OperationalError: generic SQLite error")
