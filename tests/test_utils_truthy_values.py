...
def test_env_var_enabled_uses_shared_truthy_rules(monkeypatch):
    monkeypatch.setenv("EARL_TEST_BOOL", "YeS")
    assert env_var_enabled("EARL_TEST_BOOL") is True

    monkeypatch.setenv("EARL_TEST_BOOL", "no")
    assert env_var_enabled("EARL_TEST_BOOL") is False
