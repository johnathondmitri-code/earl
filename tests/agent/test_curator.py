...
def curator_env(tmp_path, monkeypatch):
    """Isolated EARL_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".earl"
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    ...
    from earl_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `earl model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from earl_cli.main import _AUX_TASKS
    from earl_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. earl_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. earl_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
