...
def hermes_home(monkeypatch):
    """Isolate EARL_HOME for ``reload_skills`` tests.
    ...
    module-level ``EARL_HOME`` / ``SKILLS_DIR`` constants in place so the
    isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="hermes-reload-skills-")
    monkeypatch.setenv("EARL_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "EARL_HOME", home, raising=False)
