...
def test_earl_client_tag_includes_current_version():
    """The client tag must reflect earl_cli.__version__ verbatim."""
    from earl_cli import __version__
    from agent.portal_tags import earl_client_tag

    assert earl_client_tag() == f"client=hermes-client-v{__version__}"


def test_earl_client_tag_format():
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import earl_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=earl" in tags
    assert earl_client_tag() in tags
    assert len(tags) == 2
