...
Reports live under ``~/.earl/logs/curator/{YYYYMMDD-HHMMSS}/`` alongside
...
def curator_env(tmp_path, monkeypatch):
    """Isolated EARL_HOME with a skills/ dir + reset curator module state."""
    home = tmp_path / ".earl"
    home.mkdir()
    (home / "skills").mkdir()
    (home / "logs").mkdir()
...
def curator_env_with_cron(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(jobs_mod, "EARL_DIR", home)
