...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("EARL_HOME", "/users/alice/.earl/profiles/backend-worker")
        monkeypatch.setenv("EARL_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "EARL_KANBAN_DB",
            "/users/alice/.earl/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.earl/kanban/boards/smoke"]'
            in cmd
        )
