...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._hermes_home", tmp_path), \
             ...
             patch("earl_state.SessionDB", return_value=fake_db), \
             patch(
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``earl tools`` platform config for ``cron``
        ...
        # The cron default is _EARL_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``earl tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        fake_db, patches = self._make_run_job_patches(tmp_path)
        # Even if the user has ``earl tools`` configured to enable web+file
        # for cron, the per-job override wins.
        with patches[0], patches[1], patches[2], patches[3], patches[4], \
             patch("run_agent.AIAgent") as mock_agent_cls, \
             patch(
                 "earl_cli.tools_config._get_platform_tools",
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("EARL_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("EARL_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("EARL_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("EARL_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("EARL_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("EARL_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("EARL_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("EARL_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("EARL_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("EARL_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("EARL_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("EARL_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_EARL_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_EARL_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
        ...
        with patch("cron.scheduler._hermes_home", tmp_path), \
             ...
             patch("dotenv.load_dotenv"), \
             patch("earl_state.SessionDB", return_value=fake_db), \
             patch("earl_cli.runtime_provider.resolve_runtime_provider",
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_EARL_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_EARL_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_EARL_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_EARL_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_EARL_TEST_CRON_UNSET_VAR}"
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """EARL_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("EARL_CRON_MAX_PARALLEL", "1")
        call_times = []
