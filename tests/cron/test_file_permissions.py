...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("earl_cli.config.get_config_path", return_value=config_path), \
             patch("earl_cli.config.ensure_hermes_home"):
    ...
    def test_ensure_hermes_home_sets_0700(self):
        home = Path(self.tmpdir) / ".earl"
        with patch("earl_cli.config.get_hermes_home", return_value=home):
            from earl_cli.config import ensure_hermes_home
            ensure_hermes_home()
