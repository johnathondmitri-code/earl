...
2. **Isolated EARL_HOME.** EARL_HOME points to a per-test tempdir so
   code reading ``~/.earl/*`` via ``get_hermes_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".earl"`` instead of the canonical
   ``get_hermes_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No EARL_SESSION_* inheritance** — the agent's current gateway
...
# EARL_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_EARL_BEHAVIORAL_VARS = frozenset({
    "EARL_YOLO_MODE",
    "EARL_INTERACTIVE",
    "EARL_QUIET",
    "EARL_TOOL_PROGRESS",
    ...
    "EARL_SESSION_CHAT_ID",
    "EARL_SESSION_CHAT_NAME",
    "EARL_SESSION_THREAD_ID",
    "EARL_SESSION_SOURCE",
    "EARL_SESSION_KEY",
    "EARL_GATEWAY_SESSION",
    "EARL_PLATFORM",
    "EARL_MODEL",
    "EARL_INFERENCE_MODEL",
    "EARL_INFERENCE_PROVIDER",
    "EARL_TUI_PROVIDER",
    "EARL_MANAGED",
    "EARL_DEV",
    "EARL_CONTAINER",
    ...
    "EARL_TIMEZONE",
    "EARL_REDACT_SECRETS",
    "EARL_BACKGROUND_NOTIFICATIONS",
    "EARL_EXEC_ASK",
    "EARL_HOME_MODE",
    "EARL_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.earl/kanban.db instead of the per-test EARL_HOME.
    "EARL_KANBAN_DB",
    "EARL_KANBAN_BOARD",
    "EARL_KANBAN_HOME",
    "EARL_KANBAN_WORKSPACES_ROOT",
    "EARL_KANBAN_LOGS_ROOT",
    "EARL_KANBAN_TASK",
    "EARL_KANBAN_WORKSPACE",
    "EARL_KANBAN_RUN_ID",
    "EARL_KANBAN_CLAIM_LOCK",
    "EARL_KANBAN_DISPATCH_IN_GATEWAY",
    "EARL_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and EARL_HOME to per-test tempdirs so code that
    reads ``~/.earl/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral EARL_* vars that could change test semantics.
    for name in _EARL_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect EARL_HOME to a per-test tempdir. Code that reads
    #    ``~/.earl/*`` via ``get_hermes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.earl/*`` via
    #    ``Path.home() / ".earl"`` instead of ``get_hermes_home()``
    ...
    monkeypatch.setenv("EARL_HOME", str(fake_hermes_home))
    ...
    #    ~/.earl/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import earl_cli.plugins as _plugins_mod
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "earl_cli"):
...
# environment and finds the developer's live ``earl-gateway`` process
...
# correlated with a ``tests/earl_cli/`` pytest run starting up.
#
...
def _live_system_guard(request, monkeypatch):
    ...
    def _matches_hermes_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _EARL_TOKENS)
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m earl_cli.main".
                if (
                    "hermes" in low
                    or "gateway" in low
