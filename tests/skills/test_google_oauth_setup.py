...
class TestHermesConstantsFallback:
    """Tests for _hermes_home.py fallback when earl_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/.earl_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _hermes_home.py with earl_constants blocked."""
        monkeypatch.setitem(sys.modules, "earl_constants", None)
    ...
    def test_fallback_uses_hermes_home_env_var(self, monkeypatch, tmp_path):
        """When earl_constants is missing, EARL_HOME comes from env var."""
        monkeypatch.setenv("EARL_HOME", str(tmp_path / "custom-hermes"))
    ...
    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When earl_constants is missing and EARL_HOME unset, default to ~/.earl."""
        monkeypatch.delenv("EARL_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_hermes_home() == Path.home() / ".earl"

    def test_fallback_ignores_empty_hermes_home(self, monkeypatch):
        """Empty/whitespace EARL_HOME is treated as unset."""
        monkeypatch.setenv("EARL_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_hermes_home() == Path.home() / ".earl"
    ...
    def test_fallback_display_hermes_home_shortens_path(self, monkeypatch):
        ...
        monkeypatch.delenv("EARL_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "~/.earl"
    ...
    def test_fallback_display_hermes_home_profile_path(self, monkeypatch):
        ...
        monkeypatch.setenv("EARL_HOME", str(Path.home() / ".earl/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "~/.earl/profiles/coder"
    ...
    def test_fallback_display_hermes_home_custom_path(self, monkeypatch):
        ...
        monkeypatch.setenv("EARL_HOME", "/opt/hermes-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "/opt/hermes-custom"

    def test_delegates_to_earl_constants_when_available(self):
        """When earl_constants IS importable, _hermes_home delegates to it."""
        ...
        assert module.get_hermes_home is earl_constants.get_hermes_home
        assert module.display_hermes_home is earl_constants.display_hermes_home
