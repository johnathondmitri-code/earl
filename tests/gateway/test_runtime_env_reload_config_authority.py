...
EARL_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env EARL_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_max_turns(tmp_path: Path, monkeypatch) -> None:
    ...
    (hermes_home / ".env").write_text(
        "EARL_MAX_ITERATIONS=90\nOPENROUTER_API_KEY=fresh-key\n",
        encoding="utf-8",
    )

    monkeypatch.setattr(gateway_run, "_hermes_home", hermes_home)
    monkeypatch.setenv("EARL_MAX_ITERATIONS", "9000")
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["OPENROUTER_API_KEY"] == "fresh-key"
    assert os.environ["EARL_MAX_ITERATIONS"] == "9000"
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    (hermes_home / ".env").write_text("EARL_MAX_ITERATIONS=123\n", encoding="utf-8")

    monkeypatch.setattr(gateway_run, "_hermes_home", hermes_home)
    monkeypatch.delenv("EARL_MAX_ITERATIONS", raising=False)

    gateway_run._reload_runtime_env_preserving_config_authority()

    assert os.environ["EARL_MAX_ITERATIONS"] == "123"
