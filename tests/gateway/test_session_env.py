...
def test_set_session_env_sets_contextvars(monkeypatch):
    ...
    monkeypatch.delenv("EARL_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("EARL_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("EARL_SESSION_CHAT_NAME", raising=False)
    monkeypatch.delenv("EARL_SESSION_USER_ID", raising=False)
    monkeypatch.delenv("EARL_SESSION_USER_NAME", raising=False)
    monkeypatch.delenv("EARL_SESSION_THREAD_ID", raising=False)

    tokens = runner._set_session_env(context)

    # Values should be readable via get_session_env (contextvar path)
    assert get_session_env("EARL_SESSION_PLATFORM") == "telegram"
    assert get_session_env("EARL_SESSION_CHAT_ID") == "-1001"
    assert get_session_env("EARL_SESSION_CHAT_NAME") == "Group"
    assert get_session_env("EARL_SESSION_USER_ID") == "123456"
    assert get_session_env("EARL_SESSION_USER_NAME") == "alice"
    assert get_session_env("EARL_SESSION_THREAD_ID") == "17585"

    # os.environ should NOT be touched
    assert os.getenv("EARL_SESSION_PLATFORM") is None
    assert os.getenv("EARL_SESSION_THREAD_ID") is None
...
def test_clear_session_env_restores_previous_state(monkeypatch):
    ...
    assert get_session_env("EARL_SESSION_PLATFORM") == ""
    assert get_session_env("EARL_SESSION_CHAT_ID") == ""
    assert get_session_env("EARL_SESSION_CHAT_NAME") == ""
    assert get_session_env("EARL_SESSION_USER_ID") == ""
    assert get_session_env("EARL_SESSION_USER_NAME") == ""
    assert get_session_env("EARL_SESSION_THREAD_ID") == ""
...
def test_get_session_env_falls_back_to_os_environ(monkeypatch):
    ...
    monkeypatch.setenv("EARL_SESSION_PLATFORM", "discord")

    # No contextvar set — should read from os.environ
    assert get_session_env("EARL_SESSION_PLATFORM") == "discord"
...
def test_get_session_env_default_when_nothing_set(monkeypatch):
    ...
    assert get_session_env("EARL_SESSION_PLATFORM") == ""
    assert get_session_env("EARL_SESSION_PLATFORM", "fallback") == "fallback"
...
def test_session_key_set_via_contextvars(monkeypatch):
    """set_session_vars should set EARL_SESSION_KEY via contextvars."""
    monkeypatch.delenv("EARL_SESSION_KEY", raising=False)
    ...
    assert get_session_env("EARL_SESSION_KEY") == "tg:-1001:17585"

    clear_session_vars(tokens)
    assert get_session_env("EARL_SESSION_KEY") == ""


def test_session_key_falls_back_to_os_environ(monkeypatch):
    """get_session_env for SESSION_KEY should fall back to os.environ."""
    monkeypatch.setenv("EARL_SESSION_KEY", "env-session-123")

    # No contextvar set — should read from os.environ
    assert get_session_env("EARL_SESSION_KEY") == "env-session-123"

    # Set contextvar — should prefer it
    tokens = set_session_vars(session_key="ctx-session-456")
    assert get_session_env("EARL_SESSION_KEY") == "ctx-session-456"
...
def test_session_key_no_race_condition_with_contextvars(monkeypatch):
    ...
    async def handler(key: str, delay: float):
        ...
        try:
            ...
            read_back = get_session_env("EARL_SESSION_KEY")
    ...
    try:
        result = await runner._run_in_executor_with_context(
            lambda: {
                ...
                "chat_id": get_session_env("EARL_SESSION_CHAT_ID"),
                "user_id": get_session_env("EARL_SESSION_USER_ID"),
                "session_key": get_session_env("EARL_SESSION_KEY"),
            }
        )
    finally:
        runner._clear_session_env(tokens)
