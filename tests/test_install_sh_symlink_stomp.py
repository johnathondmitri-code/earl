...
symlink to the pip-generated entry point at ``$EARL_BIN`` (i.e.
...
shim's ``exec "$EARL_BIN" "$@"`` then self-recursed and ``earl`` hung on
every invocation.
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    ``EARL_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
