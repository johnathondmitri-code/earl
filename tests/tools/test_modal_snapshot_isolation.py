...
def _restore_tool_modules():
    original_hermes_home = os.environ.get("EARL_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_hermes_home is None:
            ...
            os.environ["EARL_HOME"] = original_hermes_home
        _reset_modules(("tools", "earl_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    earl_cli = types.ModuleType("earl_cli")
    earl_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["earl_cli"] = earl_cli
    hermes_home = tmp_path / "hermes-home"
    os.environ["EARL_HOME"] = str(hermes_home)
    sys.modules["earl_cli.config"] = types.SimpleNamespace(
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="earl", create_if_missing=create_if_missing)
