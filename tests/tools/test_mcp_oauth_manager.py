...
def test_hermes_provider_subclass_exists():
    ...
    from tools.mcp_oauth_manager import _EARL_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _EARL_PROVIDER_CLS is not None
    assert issubclass(_EARL_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_hermes_provider_subclass(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _EARL_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("EARL_HOME", str(tmp_path))
    ...
    assert _EARL_PROVIDER_CLS is not None
    assert isinstance(provider, _EARL_PROVIDER_CLS)
