...
class TestSendMessageTool:
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            os.environ,
            {
                "EARL_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "EARL_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
            },
            clear=False,
        ), \
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``earl cron``
    runs separately from ``earl gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``EARL_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``EARL_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``EARL_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        """Honoring EARL_KANBAN_TASK must not depend on importing or calling
        gateway.status — the worker may run with a EARL_HOME that has no
    ...
    def test_local_platform_falls_through_to_gateway_check(self, monkeypatch):
        """``EARL_SESSION_PLATFORM=local`` means CLI-style — must defer to
