...
def _clean_state():
    ...
    saved = {}
    for k in ("EARL_INTERACTIVE", "EARL_GATEWAY_SESSION", "EARL_EXEC_ASK", "EARL_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["EARL_INTERACTIVE"] = "1"
        session_key = os.getenv("EARL_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No EARL_INTERACTIVE or EARL_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
