...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_hermes_home_override(self, tmp_path):
        """Auth lookup should read from EARL_HOME/auth.json, not ~/.earl/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".earl").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``earl tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
