...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """EARL_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("EARL_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("EARL_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty EARL_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("EARL_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
