"""
Tests for timezone support (earl_time module + integration points).
...
def _reset_earl_time_cache():
    """Reset the earl_time module cache (replacement for removed reset_cache)."""
    earl_time._cached_tz = None
    earl_time._cached_tz_name = None
    earl_time._cache_resolved = False


# =========================================================================
# earl_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def setup_method(self):
        _reset_earl_time_cache()

    def teardown_method(self):
        _reset_earl_time_cache()
        os.environ.pop("EARL_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["EARL_TIMEZONE"] = "Asia/Kolkata"
        result = earl_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["EARL_TIMEZONE"] = "UTC"
        result = earl_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["EARL_TIMEZONE"] = "America/New_York"
        result = earl_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["EARL_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="earl_time"):
            result = earl_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["EARL_TIMEZONE"] = "Europe/London"
        tz = earl_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["EARL_TIMEZONE"] = "Not/A/Timezone"
        tz = earl_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When EARL_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) EARL_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("EARL_TIMEZONE=" + os.environ.get("EARL_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "EARL_TIMEZONE=NOT_SET" in result["output"], (
            "EARL_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When EARL_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("EARL_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        os.environ["EARL_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_earl_time_cache()
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["EARL_TIMEZONE"] = "US/Eastern"
        _reset_earl_time_cache()
