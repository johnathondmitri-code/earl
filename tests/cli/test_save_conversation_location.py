...
Regression: the old implementation wrote ``earl_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``earl sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.earl/sessions/saved/`` and prints
...
def hermes_home(tmp_path, monkeypatch):
    ...
    if hasattr(earl_constants, "_hermes_home_cache"):
        earl_constants._hermes_home_cache = None
    return home
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.earl/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the EARL_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "earl_constants"]:
    ...
    # File MUST be under ~/.earl/sessions/saved/
