...
def hermes_home(tmp_path, monkeypatch):
    """Isolated EARL_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".earl"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("EARL_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves EARL_HOME each test.
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, hermes_home):
        ...
        with patch("earl_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, hermes_home):
        ...
        with patch("earl_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
