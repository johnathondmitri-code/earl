"""Tests for earl_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Hermes entry point
(hermes, earl, hermes-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports earl_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported earl_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("earl_bootstrap", None)
    import earl_bootstrap  # noqa: WPS433
    return earl_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import earl_bootstrap as its
    ...
    # import earl_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "earl_cli/main.py",   # hermes CLI (console_script)
        "run_agent.py",          # earl (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import earl_bootstrap' and that
        ...
        half-finished ``earl update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``earl_bootstrap`` as a top-level module) leaves hermes
        ...
        """
        # Resolve relative to the earl repo root.  Tests live
        # at tests/test_earl_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when earl_bootstrap hasn't been
        ...
        assert first_import_name == "earl_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'earl_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import earl_bootstrap' line to be the first import."
        )
