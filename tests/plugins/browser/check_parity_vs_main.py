...
Run from the PR worktree:
    ...
    cd ~/.earl/earl/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.earl/earl``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.earl/earl
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a earl checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a earl checkout"
)
...
# Isolated EARL_HOME for the config write.
