...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["repository"] == "https://github.com/NousResearch/earl"
    assert data["website"].startswith("https:/.earl-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"earl[acp]=={data['version']}"
    assert uvx["args"] == ["earl-acp"]
