...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["EARL_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("EARL_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("EARL_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("EARL_VOICE", "1")
    monkeypatch.delenv("EARL_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("EARL_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("EARL_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "earl_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("EARL_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("EARL_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "earl_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import earl_cli.config as config_mod
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("EARL_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "earl_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("EARL_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("EARL_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("EARL_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("EARL_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "earl_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown EARL_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("EARL_MODEL", raising=False)
    monkeypatch.setenv("EARL_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("EARL_MODEL", raising=False)
    monkeypatch.delenv("EARL_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("EARL_MODEL", "nous/hermes-test")
    monkeypatch.setenv("EARL_TUI_PROVIDER", "nous")
    monkeypatch.delenv("EARL_INFERENCE_PROVIDER", raising=False)
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("EARL_MODEL", "nous/hermes-test")
    monkeypatch.delenv("EARL_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("EARL_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "earl_cli.models.detect_static_provider_for_model",
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("EARL_MODEL", "sonnet")
    monkeypatch.delenv("EARL_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("EARL_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "earl_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "earl_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "earl_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "earl_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("EARL_YOLO_MODE", "false")
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("EARL_EXEC_ASK", raising=False)
    monkeypatch.delenv("EARL_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("EARL_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["EARL_GATEWAY_SESSION"] == "1"
    assert server.os.environ["EARL_EXEC_ASK"] == "1"
    assert server.os.environ["EARL_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("earl_cli.main._has_any_provider_configured", lambda: False)
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("earl_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("earl_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    """After an explicit provider switch, EARL_INFERENCE_PROVIDER must
    ...
    server._sessions["sid"] = _session(agent=_Agent())
    monkeypatch.setenv("EARL_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "earl_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    assert os.environ["EARL_INFERENCE_PROVIDER"] == "anthropic"


def test_config_set_model_syncs_tui_provider_unconditionally(monkeypatch):
    """Regression for #16857: /model must set EARL_TUI_PROVIDER even when
    ...
    # Both env vars must reflect the user's choice. EARL_TUI_PROVIDER is
    # the canonical explicit-this-process carrier consumed by
    # _resolve_startup_runtime() on /new.
    assert os.environ["EARL_TUI_PROVIDER"] == "custom:xuanji"
    assert os.environ["EARL_INFERENCE_PROVIDER"] == "custom:xuanji"
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    agent = Agent()
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setenv("EARL_TUI_PROVIDER", "openai-codex")
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("earl_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert resp["result"]["value"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["EARL_TUI_PROVIDER"] == "anthropic"
        assert os.environ["EARL_MODEL"] == "anthropic/claude-sonnet-4.6"
        assert os.environ["EARL_INFERENCE_MODEL"] == "anthropic/claude-sonnet-4.6"
    finally:
        server._sessions.clear()
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("earl_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "earl_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("earl_state")
    ...
    monkeypatch.setitem(sys.modules, "earl_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # fixture pins EARL_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `earl model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `earl model` and the classic CLI /model picker.
    ...
    with patch(
        "earl_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("earl_cli.models.provider_model_ids") as live_fetch:
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"earl_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "earl_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "earl_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "earl_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_earl_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.earl/.env
    ...
    with patch.dict(sys.modules, {"earl_cli.config": fake}):
