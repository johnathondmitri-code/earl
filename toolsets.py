...
_EARL_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (EARL_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `earl tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (EARL_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Earl toolsets (CLI + messaging platforms)
    #
    ...
    "earl-cli": {
        ...
        "tools": _EARL_CORE_TOOLS,
        "includes": []
    },

    "earl-cron": {
        # Mirrors earl-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `earl tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as earl-cli; gated by `earl tools`",
        "tools": _EARL_CORE_TOOLS,
        "includes": []
    },
    ...
    "earl-email": {
        "description": "Email bot toolset - interact with Earl via email (IMAP/SMTP)",
        "tools": _EARL_CORE_TOOLS,
        "includes": []
    },
    ...
    "earl-sms": {
        "description": "SMS bot toolset - interact with Earl via SMS (Twilio)",
        "tools": _EARL_CORE_TOOLS,
        "includes": []
    },
    ...
    "earl-gateway": {
        ...
        "tools": [],
        "includes": ["earl-telegram", "earl-discord", "earl-whatsapp", "earl-slack", "earl-signal", "earl-bluebubbles", "earl-homeassistant", "earl-email", "earl-sms", "earl-mattermost", "earl-matrix", "earl-dingtalk", "earl-feishu", "earl-wecom", "earl-wecom-callback", "earl-weixin", "earl-qqbot", "earl-webhook", "earl-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (earl-<name>).
        # Gives them _EARL_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("earl-"):
            platform_name = name[len("earl-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_EARL_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
