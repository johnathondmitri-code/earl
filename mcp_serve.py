"""
Earl MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Earl-specific extra)

Usage:
    earl mcp serve
    earl mcp serve --verbose
...
logger = logging.getLogger("earl.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using EARL_HOME."""
    try:
        from earl_constants import get_earl_home
        return get_earl_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("EARL_HOME", Path.home() / ".earl")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from earl_constants import get_earl_home
        directory_file = get_earl_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("EARL_HOME", Path.home() / ".earl")
...
class EventBridge:
    ...
    This is the Earl equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from earl_constants import get_earl_home
            db_file = get_earl_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("EARL_HOME", Path.home() / ".earl")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Earl MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "earl",
        instructions=(
            "Earl Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Earl MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
